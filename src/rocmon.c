...
#define ROCM_CALL(handleerror, func, ...)                                                          \
    do {                                                                                           \
        ...
        if (s != HSA_STATUS_SUCCESS && s != HSA_STATUS_INFO_BREAK) {                               \
            ...
            ERROR_PRINT("Error: function %s failed with error: '%s' (hsa_status_t=%d).",           \
                #func,                                                                             \
                errstr,                                                                            \
                s);                                                                                \
            handleerror;                                                                           \
...
#define RSMI_CALL(handleerror, func, ...)                                                          \
    do {                                                                                           \
        ...
        if (s != RSMI_STATUS_SUCCESS) {                                                            \
            ...
            ERROR_PRINT("Error: function %s failed with error: '%s' (rsmi_status_t=%d)",           \
                #func,                                                                             \
                errstr,                                                                            \
                s);                                                                                \
            handleerror;                                                                           \
...
#define DECLAREFUNC_HSA(funcname, ...)                                                             \
    hsa_status_t __attribute__((weak)) funcname(__VA_ARGS__);                                      \
    static hsa_status_t (*funcname##_ptr)(__VA_ARGS__);
#define DECLAREFUNC_SMI(funcname, ...)                                                             \
    rsmi_status_t __attribute__((weak)) funcname(__VA_ARGS__);                                     \
    static rsmi_status_t (*funcname##_ptr)(__VA_ARGS__);
...
DECLAREFUNC_HSA(rocprofiler_iterate_info, const hsa_agent_t *agent, rocprofiler_info_kind_t kind,
...
DECLAREFUNC_HSA(rocprofiler_open, hsa_agent_t agent, rocprofiler_feature_t *features,
    uint32_t feature_count, rocprofiler_t **context, uint32_t mode,
...
DECLAREFUNC_SMI(rsmi_dev_supported_variant_iterator_open, rsmi_func_id_iter_handle_t obj_h,
...
DECLAREFUNC_SMI(rsmi_dev_pci_throughput_get, uint32_t dv_ind, uint64_t *sent, uint64_t *received,
    uint64_t *max_pkt_sz);
...
DECLAREFUNC_SMI(rsmi_dev_memory_reserved_pages_get, uint32_t dv_ind, uint32_t *num_pages,
...
DECLAREFUNC_SMI(rsmi_dev_temp_metric_get, uint32_t dv_ind, uint32_t sensor_type,
    ...
    int deviceId, RocmonSmiEvent *event, RocmonEventResult *result)
{
    ...
    if (event->extra == 0)
        RSMI_CALL(return -1, rsmi_dev_pci_throughput_get, deviceId, &value, NULL, NULL);
    else if (event->extra == 1)
        RSMI_CALL(return -1, rsmi_dev_pci_throughput_get, deviceId, NULL, &value, NULL);
    else if (event->extra == 2)
        RSMI_CALL(return -1, rsmi_dev_pci_throughput_get, deviceId, NULL, NULL, &value);
    else
        return -1;
    ...
    int deviceId, RocmonSmiEvent *event, RocmonEventResult *result)
{
    int64_t voltage;
    RSMI_CALL(
        return -1, rsmi_dev_volt_metric_get, deviceId, event->subvariant, event->variant, &voltage);
...
static hsa_status_t _rocmon_iterate_agents_callback(hsa_agent_t agent, void *argv)
{
    ...
    ROCM_CALL(return HSA_STATUS_ERROR,
        rocprofiler_iterate_info,
        &agent,
        ROCPROFILER_INFO_KIND_METRIC,
        _rocmon_iterate_info_callback_count,
        device);
    ...
    if (noAgent) {
        ROCM_CALL(return HSA_STATUS_ERROR,
            ...
            NULL,
            ROCPROFILER_INFO_KIND_METRIC,
            _rocmon_iterate_info_callback_add,
            &info_arg);
    } else {
        ROCM_CALL(return HSA_STATUS_ERROR,
            rocprofiler_iterate_info,
...
int rocmon_init(int numGpus, const int *gpuIds)
{
    ...
    ROCM_CALL(ERROR_PRINT("Failed to get HSA timestamp factor");
    ...
    ROCM_CALL(ERROR_PRINT("Error while iterating through available agents");
        goto rocmon_init_info_agents_failed,
        hsa_iterate_agents,
        _rocmon_iterate_agents_callback,
        &arg);
