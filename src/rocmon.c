...
#if AMDSMI_LIB_VERSION_YEAR == 23 && AMDSMI_LIB_VERSION_MAJOR == 4 &&                              \
    AMDSMI_LIB_VERSION_MINOR == 0 && AMDSMI_LIB_VERSION_RELEASE == 0
...
#define DECLAREFUNC_HSA(funcname, funcsig)                                                         \
    hsa_status_t ROCMWEAK funcname funcsig;                                                        \
    hsa_status_t(*funcname##_ptr) funcsig;
#define DECLAREFUNC_SMI(funcname, funcsig)                                                         \
    rsmi_status_t ROCMWEAK funcname funcsig;                                                       \
    rsmi_status_t(*funcname##_ptr) funcsig;

DECLAREFUNC_HSA(hsa_init, ());
DECLAREFUNC_HSA(hsa_shut_down, ());
DECLAREFUNC_HSA(
    hsa_iterate_agents, (hsa_status_t(*callback)(hsa_agent_t agent, void *data), void *data));
...
DECLAREFUNC_HSA(rocprofiler_iterate_info,
    (const hsa_agent_t *agent, rocprofiler_info_kind_t kind,
        hsa_status_t (*callback)(const rocprofiler_info_data_t, void *data), void *data));
DECLAREFUNC_HSA(rocprofiler_close, (rocprofiler_t * context));
DECLAREFUNC_HSA(rocprofiler_open,
    (hsa_agent_t agent, rocprofiler_feature_t *features, uint32_t feature_count,
        rocprofiler_t **context, uint32_t mode, rocprofiler_properties_t *properties));
...
DECLAREFUNC_SMI(
    rsmi_dev_supported_func_iterator_open, (uint32_t dv_ind, rsmi_func_id_iter_handle_t *handle));
DECLAREFUNC_SMI(rsmi_dev_supported_variant_iterator_open,
    (rsmi_func_id_iter_handle_t obj_h, rsmi_func_id_iter_handle_t *var_iter));
DECLAREFUNC_SMI(
    rsmi_func_iter_value_get, (rsmi_func_id_iter_handle_t handle, rsmi_func_id_value_t *value));
...
DECLAREFUNC_SMI(rsmi_dev_pci_throughput_get,
    (uint32_t dv_ind, uint64_t *sent, uint64_t *received, uint64_t *max_pkt_sz));
...
DECLAREFUNC_SMI(rsmi_dev_memory_reserved_pages_get,
    (uint32_t dv_ind, uint32_t *num_pages, rsmi_retired_page_record_t *records));
...
DECLAREFUNC_SMI(rsmi_dev_volt_metric_get, (uint32_t dv_ind, rsmi_voltage_type_t sensor_type,
                                              rsmi_voltage_metric_t metric, int64_t *voltage));
DECLAREFUNC_SMI(rsmi_dev_overdrive_level_get, (uint32_t dv_ind, uint32_t *od));
DECLAREFUNC_SMI(
    rsmi_dev_ecc_count_get, (uint32_t dv_ind, rsmi_gpu_block_t block, rsmi_error_count_t *ec));
...
static int _smi_wrapper_pci_throughput_get(
    int deviceId, RocmonSmiEvent *event, RocmonEventResult *result)
{
    uint64_t value;
    ROCMON_DEBUG_PRINT(
        DEBUGLEV_DEVELOP, "_smi_wrapper_pci_throughput_get(%d, %d)", deviceId, event->extra);
    // Internal variant: 0 for sent, 1 for received bytes and 2 for max packet size
    if (event->extra == 0)
        RSMI_CALL(rsmi_dev_pci_throughput_get, (deviceId, &value, NULL, NULL), return -1);
    else if (event->extra == 1)
        RSMI_CALL(rsmi_dev_pci_throughput_get, (deviceId, NULL, &value, NULL), return -1);
    else if (event->extra == 2)
        RSMI_CALL(rsmi_dev_pci_throughput_get, (deviceId, NULL, NULL, &value), return -1);
    else
        return -1;
...
static int _smi_wrapper_pci_replay_counter_get(
    int deviceId, RocmonSmiEvent *event, RocmonEventResult *result)
{
    uint64_t counter;
...
static int _smi_wrapper_power_ave_get(
    int deviceId, RocmonSmiEvent *event, RocmonEventResult *result)
{
    uint64_t power;
...
static int _smi_wrapper_memory_total_get(
    int deviceId, RocmonSmiEvent *event, RocmonEventResult *result)
{
    uint64_t total;
...
static int _smi_wrapper_memory_usage_get(
    int deviceId, RocmonSmiEvent *event, RocmonEventResult *result)
{
    uint64_t used;
...
static int _smi_wrapper_memory_busy_percent_get(
    int deviceId, RocmonSmiEvent *event, RocmonEventResult *result)
{
    uint32_t percent;
...
static int _smi_wrapper_memory_reserved_pages_get(
    int deviceId, RocmonSmiEvent *event, RocmonEventResult *result)
{
    uint32_t num_pages;
...
static int _smi_wrapper_fan_rpms_get(int deviceId, RocmonSmiEvent *event, RocmonEventResult *result)
{
    int64_t speed;
...
static int _smi_wrapper_fan_speed_get(
    int deviceId, RocmonSmiEvent *event, RocmonEventResult *result)
{
    int64_t speed;
...
static int _smi_wrapper_fan_speed_max_get(
    int deviceId, RocmonSmiEvent *event, RocmonEventResult *result)
{
    int64_t max_speed;
...
static int _smi_wrapper_temp_metric_get(
    int deviceId, RocmonSmiEvent *event, RocmonEventResult *result)
{
    int64_t temperature;
    RSMI_CALL(rsmi_dev_temp_metric_get,
        (deviceId, event->subvariant, event->variant, &temperature),
        return -1);
...
static int _smi_wrapper_volt_metric_get(
    int deviceId, RocmonSmiEvent *event, RocmonEventResult *result)
{
    int64_t voltage;
    RSMI_CALL(rsmi_dev_volt_metric_get,
        (deviceId, event->subvariant, event->variant, &voltage),
        return -1);
...
static int _smi_wrapper_overdrive_level_get(
    int deviceId, RocmonSmiEvent *event, RocmonEventResult *result)
{
    uint32_t overdrive;
...
static int _smi_wrapper_ecc_count_get(
    int deviceId, RocmonSmiEvent *event, RocmonEventResult *result)
{
...
static int _smi_wrapper_compute_process_info_get(
    int deviceId, RocmonSmiEvent *event, RocmonEventResult *result)
{
    uint32_t num_items;
...
#define DLSYM_AND_CHECK(dllib, name)                                                               \
    ...
    if (dlerror() != NULL) {                                                                       \
        ERROR_PRINT("Failed to link " #name);                                                      \
        return -1;                                                                                 \
    }
...
static hsa_status_t _rocmon_iterate_info_callback_count(
    const rocprofiler_info_data_t info, void *data)
{
...
static void _rocmon_print_rocprofiler_info_data(const rocprofiler_info_data_t info)
{
...
static hsa_status_t _rocmon_iterate_info_callback_add(
    const rocprofiler_info_data_t info, void *data)
{
...
static hsa_status_t _rocmon_iterate_agents_callback(hsa_agent_t agent, void *argv)
{
    ...
    ROCM_CALL(rocprofiler_iterate_info,
        (&agent, ROCPROFILER_INFO_KIND_METRIC, _rocmon_iterate_info_callback_count, device),
        return HSA_STATUS_ERROR);
    ...
    if (device->numRocMetrics == 0) {
        ROCM_CALL(rocprofiler_iterate_info,
            (NULL, ROCPROFILER_INFO_KIND_METRIC, _rocmon_iterate_info_callback_count, device),
            return HSA_STATUS_ERROR);
        noAgent = true;
    }
    ...
    device->rocMetrics =
        (rocprofiler_info_data_t *)malloc(device->numRocMetrics * sizeof(rocprofiler_info_data_t));
    ...
    ROCMON_DEBUG_PRINT(DEBUGLEV_INFO,
        "Read %d RocProfiler events for device %d",
        device->numRocMetrics,
        device->deviceId);
    ...
    if (noAgent) {
        ROCM_CALL(rocprofiler_iterate_info,
            (NULL, ROCPROFILER_INFO_KIND_METRIC, _rocmon_iterate_info_callback_add, &info_arg),
            return HSA_STATUS_ERROR);
    } else {
        ROCM_CALL(rocprofiler_iterate_info,
            (&agent, ROCPROFILER_INFO_KIND_METRIC, _rocmon_iterate_info_callback_add, &info_arg),
            return HSA_STATUS_ERROR);
    }
...
static int _rocmon_parse_eventstring(const char *eventString, GroupInfo *group)
{
    int err                = 0;
    ...
    if (bstrchrp(eventBString, ':', 0) != BSTR_ERR) {
        ...
        if (err == -EACCES) {
            ...
            ERROR_PRINT(
                "Performance group %s only available with deactivated HyperThreading", eventString);
            return err;
        }
        if (err < 0) {
...
static int _rocmon_get_timestamp(uint64_t *timestamp_ns)
{
    uint64_t timestamp;
...
static int _rocmon_getLastResult(RocmonDevice *device, int eventId, double *value)
{
...
static int _rocmon_readCounters_rocprofiler(RocmonDevice *device)
{
    int ret;
...
static int _rocmon_readCounters_smi(RocmonDevice *device)
{
    // Check if there are any counters to start
...
static int _rocmon_readCounters(uint64_t *(*getDestTimestampFunc)(RocmonDevice *device))
{
    int ret;
...
static uint64_t *_rocmon_get_read_time(RocmonDevice *device) { return &device->time.read; }

static uint64_t *_rocmon_get_stop_time(RocmonDevice *device) { return &device->time.stop; }

// ----------------------------------------------------
//   Rocmon SMI helper functions
// ----------------------------------------------------

static bstring _rocmon_smi_build_label(
    RocmonSmiEventType type, const char *funcname, uint64_t variant, uint64_t subvariant)
{
    switch (type) {
...
static int _rocmon_smi_add_event_to_device(RocmonDevice *device, const char *funcname,
    RocmonSmiEventType type, int64_t variant, uint64_t subvariant)
{
    int ret;
...
static int _rocmon_smi_get_function_subvariants(RocmonDevice *device, const char *funcname,
    uint64_t variant, rsmi_func_id_iter_handle_t var_iter)
{
    ...
    if (status == RSMI_STATUS_NO_DATA) {
        // No subvariants
        ret = _rocmon_smi_add_event_to_device(
            device, funcname, ROCMON_SMI_EVENT_TYPE_VARIANT, variant, 0);
        if (ret < 0)
            return -1;
        return 0;
    }
    ...
    do {
        ...
        if (variant == RSMI_DEFAULT_VARIANT)
            ret = _rocmon_smi_add_event_to_device(
                device, funcname, ROCMON_SMI_EVENT_TYPE_INSTANCES, variant, value.id);
        else
            ret = _rocmon_smi_add_event_to_device(
                device, funcname, ROCMON_SMI_EVENT_TYPE_SUBVARIANT, variant, value.id);
        if (ret < 0)
            return ret;
...
static int _rocmon_smi_get_function_variants(
    RocmonDevice *device, const char *funcname, rsmi_func_id_iter_handle_t iter_handle)
{
...
static int _rocmon_smi_get_functions(RocmonDevice *device)
{
    ...
    RSMI_CALL(
        rsmi_dev_supported_func_iterator_open, (device->deviceId, &iter_handle), { return -1; });
    ...
    ret = _rocmon_smi_add_event_to_device(
        device, "rsmi_compute_process_info_get", ROCMON_SMI_EVENT_TYPE_NORMAL, 0, 0);
    if (ret < 0)
        return -1;
...
#define ADD_SMI_EVENT(name, type, smifunc, variant, subvariant, extra, measurefunc)                \
    if (_rocmon_smi_add_event_to_map(                                                              \
            name, type, smifunc, variant, subvariant, extra, measurefunc) < 0) {                   \
        return -1;                                                                                 \
    }
...
#define ADD_SMI_EVENT_S(name, smifunc, variant, subvariant, extra, measurefunc)                    \
    ADD_SMI_EVENT(                                                                                 \
        name, ROCMON_SMI_EVENT_TYPE_SUBVARIANT, smifunc, variant, subvariant, extra, measurefunc)
#define ADD_SMI_EVENT_I(name, smifunc, extra, measurefunc)                                         \
    ADD_SMI_EVENT(name, ROCMON_SMI_EVENT_TYPE_INSTANCES, smifunc, 0, 0, extra, measurefunc)

static int _rocmon_smi_add_event_to_map(char *name, RocmonSmiEventType type, char *smifunc,
    uint64_t variant, uint64_t subvariant, uint64_t extra, RocmonSmiMeasureFunc measureFunc)
{
    ...
    list->numEntries++;
    list->entries =
        (RocmonSmiEvent *)realloc(list->entries, list->numEntries * sizeof(RocmonSmiEvent));
...
static void _rcomon_smi_free_event_list(void *vlist)
{
...
static int _rocmon_smi_init_events()
{
    int ret;
    ...
    // Add events
    ADD_SMI_EVENT_N(
        "PCI_THROUGHPUT_SENT", "rsmi_dev_pci_throughput_get", 0, &_smi_wrapper_pci_throughput_get);
    ADD_SMI_EVENT_N("PCI_THROUGHPUT_RECEIVED",
        "rsmi_dev_pci_throughput_get",
        1,
        &_smi_wrapper_pci_throughput_get);
    ADD_SMI_EVENT_N("PCI_THROUGHPUT_MAX_PKT_SZ",
        "rsmi_dev_pci_throughput_get",
        2,
        &_smi_wrapper_pci_throughput_get);
    ADD_SMI_EVENT_N("PCI_REPLAY_COUNTER",
        "rsmi_dev_pci_replay_counter_get",
        0,
        &_smi_wrapper_pci_replay_counter_get);
    ADD_SMI_EVENT_I("POWER_AVE", "rsmi_dev_power_ave_get", 0, &_smi_wrapper_power_ave_get);
    ADD_SMI_EVENT_V("MEMORY_TOTAL_VRAM",
        "rsmi_dev_memory_total_get",
        RSMI_MEM_TYPE_VRAM,
        0,
        &_smi_wrapper_memory_total_get);
    ADD_SMI_EVENT_V("MEMORY_TOTAL_VIS_VRAM",
        "rsmi_dev_memory_total_get",
        RSMI_MEM_TYPE_VIS_VRAM,
        0,
        &_smi_wrapper_memory_total_get);
    ADD_SMI_EVENT_V("MEMORY_TOTAL_GTT",
        "rsmi_dev_memory_total_get",
        RSMI_MEM_TYPE_GTT,
        0,
        &_smi_wrapper_memory_total_get);
    ADD_SMI_EVENT_V("MEMORY_USAGE_VRAM",
        "rsmi_dev_memory_usage_get",
        RSMI_MEM_TYPE_VRAM,
        0,
        &_smi_wrapper_memory_usage_get);
    ADD_SMI_EVENT_V("MEMORY_USAGE_VIS_VRAM",
        "rsmi_dev_memory_usage_get",
        RSMI_MEM_TYPE_VIS_VRAM,
        0,
        &_smi_wrapper_memory_usage_get);
    ADD_SMI_EVENT_V("MEMORY_USAGE_GTT",
        "rsmi_dev_memory_usage_get",
        RSMI_MEM_TYPE_GTT,
        0,
        &_smi_wrapper_memory_usage_get);
    ADD_SMI_EVENT_N("MEMORY_BUSY_PERCENT",
        "rsmi_dev_memory_busy_percent_get",
        0,
        &_smi_wrapper_memory_busy_percent_get);
    ADD_SMI_EVENT_N("MEMORY_NUM_RESERVED_PAGES",
        "rsmi_dev_memory_reserved_pages_get",
        0,
        &_smi_wrapper_memory_reserved_pages_get);
    ...
    ADD_SMI_EVENT_I(
        "FAN_SPEED_MAX", "rsmi_dev_fan_speed_max_get", 0, &_smi_wrapper_fan_speed_max_get);
    ADD_SMI_EVENT_S("TEMP_EDGE",
        "rsmi_dev_temp_metric_get",
        RSMI_TEMP_CURRENT,
        RSMI_TEMP_TYPE_EDGE,
        0,
        &_smi_wrapper_temp_metric_get);
    ADD_SMI_EVENT_S("TEMP_JUNCTION",
        "rsmi_dev_temp_metric_get",
        RSMI_TEMP_CURRENT,
        RSMI_TEMP_TYPE_JUNCTION,
        0,
        &_smi_wrapper_temp_metric_get);
    ADD_SMI_EVENT_S("TEMP_MEMORY",
        "rsmi_dev_temp_metric_get",
        RSMI_TEMP_CURRENT,
        RSMI_TEMP_TYPE_MEMORY,
        0,
        &_smi_wrapper_temp_metric_get);
    ADD_SMI_EVENT_S("VOLT_VDDGFX",
        "rsmi_dev_volt_metric_get",
        RSMI_VOLT_CURRENT,
        RSMI_VOLT_TYPE_VDDGFX,
        0,
        &_smi_wrapper_volt_metric_get);
    ADD_SMI_EVENT_N(
        "OVERDRIVE_LEVEL", "rsmi_dev_overdrive_level_get", 0, &_smi_wrapper_overdrive_level_get);
    ADD_SMI_EVENT_V("ECC_COUNT_UMC_CORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_UMC,
        0,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_V("ECC_COUNT_UMC_UNCORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_UMC,
        1,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_V("ECC_COUNT_SDMA_CORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_SDMA,
        0,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_V("ECC_COUNT_SDMA_UNCORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_SDMA,
        1,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_V("ECC_COUNT_GFX_CORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_GFX,
        0,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_V("ECC_COUNT_GFX_UNCORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_GFX,
        1,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_V("ECC_COUNT_MMHUB_CORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_MMHUB,
        0,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_V("ECC_COUNT_MMHUB_UNCORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_MMHUB,
        1,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_V("ECC_COUNT_ATHUB_CORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_ATHUB,
        0,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_V("ECC_COUNT_ATHUB_UNCORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_ATHUB,
        1,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_V("ECC_COUNT_PCIE_BIF_CORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_PCIE_BIF,
        0,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_V("ECC_COUNT_PCIE_BIF_UNCORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_PCIE_BIF,
        1,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_V("ECC_COUNT_HDP_CORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_HDP,
        0,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_V("ECC_COUNT_HDP_UNCORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_HDP,
        1,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_V("ECC_COUNT_XGMI_WAFL_CORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_XGMI_WAFL,
        0,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_V("ECC_COUNT_XGMI_WAFL_UNCORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_XGMI_WAFL,
        1,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_V("ECC_COUNT_DF_CORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_DF,
        0,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_V("ECC_COUNT_DF_UNCORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_DF,
        1,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_V("ECC_COUNT_SMN_CORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_SMN,
        0,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_V("ECC_COUNT_SMN_UNCORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_SMN,
        1,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_V("ECC_COUNT_SEM_CORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_SEM,
        0,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_V("ECC_COUNT_SEM_UNCORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_SEM,
        1,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_V("ECC_COUNT_MP0_CORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_MP0,
        0,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_V("ECC_COUNT_MP0_UNCORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_MP0,
        1,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_V("ECC_COUNT_MP1_CORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_MP1,
        0,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_V("ECC_COUNT_MP1_UNCORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_MP1,
        1,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_V("ECC_COUNT_FUSE_CORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_FUSE,
        0,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_V("ECC_COUNT_FUSE_UNCORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_FUSE,
        1,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_V("ECC_COUNT_LAST_CORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_LAST,
        0,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_V("ECC_COUNT_LAST_UNCORRECTABLE",
        "rsmi_dev_ecc_count_get",
        RSMI_GPU_BLOCK_LAST,
        1,
        &_smi_wrapper_ecc_count_get);
    ADD_SMI_EVENT_N("PROCS_USING_GPU",
        "rsmi_compute_process_info_get",
        0,
        &_smi_wrapper_compute_process_info_get);
...
int rocmon_init(int numGpus, const int *gpuIds)
{
    ...
    for (int i = 0; i < rocmon_context->numDevices; i++) {
        if (_rocmon_smi_get_functions(&rocmon_context->devices[i]) < 0) {
            ERROR_PRINT(
                "Failed to get SMI functions for device %d", rocmon_context->devices[i].deviceId);
            goto rocmon_init_info_agents_failed;
        }
    }
...
int rocmon_addEventSet(const char *eventString, int *gid)
{
    ...
    if (rocmon_context->numActiveGroups == rocmon_context->numGroups) {
        GroupInfo *tmpInfo = (GroupInfo *)realloc(
            rocmon_context->groups, (rocmon_context->numGroups + 1) * sizeof(GroupInfo));
        if (tmpInfo == NULL) {
    ...
    // Parse event string
    int err = _rocmon_parse_eventstring(
        eventString, &rocmon_context->groups[rocmon_context->numActiveGroups]);
    if (err < 0) {
        return err;
    }
    ...
    for (int i = 0; i < rocmon_context->numDevices; i++) {
        ...
        RocmonEventResult *tmpResults =
            (RocmonEventResult *)malloc(numEvents * sizeof(RocmonEventResult));
        if (tmpResults == NULL) {
        ...
        RocmonEventResultList *tmpGroupResults = (RocmonEventResultList *)realloc(
            device->groupResults, (device->numGroupResults + 1) * sizeof(RocmonEventResultList));
...
static int _rocmon_setupCounters_rocprofiler(
    RocmonDevice *device, const char **events, int numEvents)
{
    ...
    rocprofiler_feature_t *features =
        (rocprofiler_feature_t *)malloc(numEvents * sizeof(rocprofiler_feature_t));
    if (features == NULL) {
    ...
    ROCM_CALL(rocprofiler_open,
        (device->hsa_agent,
            device->activeRocEvents,
            device->numActiveRocEvents,
            &device->context,
            mode,
            &properties),
        return -1);
...
static int _rocmon_setupCounters_smi(RocmonDevice *device, const char **events, int numEvents)
{
    int ret;
    ...
    for (int i = 0; i < numEvents; i++) {
        ...
        if (instance >= 0 && instance >= metric->instances) {
            ERROR_PRINT("Instance %d seleced but event '%s' has only %d",
                instance,
                eventName,
                metric->instances);
            free(activeEvents);
...
static int _rocmon_startCounters_rocprofiler(RocmonDevice *device)
{
    // Check if there are any counters to start
    if (device->numActiveRocEvents <= 0) {
        return 0;
...
static int _rocmon_startCounters_smi(RocmonDevice *device)
{
    // Check if there are any counters to start
    if (device->numActiveSmiEvents <= 0) {
        return 0;
...
static int _rocmon_stopCounters_rocprofiler(RocmonDevice *device)
{
    if (device->context) {
        // Close context
...
{
    if (!rocmon_context || !rocmon_initialized || (groupId < 0) ||
        groupId >= rocmon_context->numActiveGroups) {
        return -EFAULT;
    }
