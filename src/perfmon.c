...
int (*perfmon_startCountersThread)(int thread_id,
                                   PerfmonEventSet *eventSet) = NULL;
int (*perfmon_stopCountersThread)(int thread_id,
                                  PerfmonEventSet *eventSet) = NULL;
int (*perfmon_readCountersThread)(int thread_id,
                                  PerfmonEventSet *eventSet) = NULL;
int (*perfmon_setupCountersThread)(int thread_id,
                                   PerfmonEventSet *eventSet) = NULL;
int (*perfmon_finalizeCountersThread)(int thread_id,
                                      PerfmonEventSet *eventSet) = NULL;
...
static int getIndexAndType(bstring reg, RegisterIndex *index,
                           RegisterType *type) {
  int ret = FALSE;
...
static RegisterType checkAccess(bstring reg, RegisterIndex index,
                                RegisterType oldtype, int force) {
  int err = 0;
  uint64_t tmp = 0x0ULL;
  ...
  if (cpuid_info.isIntel && type == PMC &&
      (index - firstpmcindex) >= cpuid_info.perf_num_ctr) {
    fprintf(stderr,
            "WARN: Counter %s is only available with deactivated "
  ...
  if (type == NOTYPE) {
    DEBUG_PRINT(DEBUGLEV_INFO,
                "WARNING: Counter %s not available on the current system. "
                "Counter results defaults to 0.",
                bdata(reg));
    return NOTYPE;
  }
  checkName = bfromcstr(counter_map[index].key);
  if (bstrcmp(reg, checkName) != BSTR_OK) {
    DEBUG_PRINT(DEBUGLEV_INFO, "WARNING: Counter %s does not exist",
                bdata(reg));
    bdestroy(checkName);
    return NOTYPE;
  }
  ...
  if (!err) {
    DEBUG_PRINT(DEBUGLEV_INFO,
                ...
                pci_device_names[counter_map[index].device], bdata(reg));
    return NOTYPE;
  }
  if ((type != THERMAL) && (type != VOLTAGE) && (type != POWER) &&
      (type != WBOX0FIX)) {
    ...
    if ((check_settings) && (tmp != 0x0ULL)) {
      if (force == 1 || groupSet->numberOfGroups > 1) {
        DEBUG_PRINT(DEBUGLEV_DETAIL,
                    "Counter %s has bits set (0x%llx) but we are forced to "
                    "overwrite them",
                    counter_map[index].key, LLU_CAST tmp);
        /*                err = HPMwrite(testcpu, counter_map[index].device,
         * reg, 0x0ULL);*/
        ...
        /*                }*/
      } else if ((force == 0) && ((type != FIXED) && (type != THERMAL) &&
                                  (type != VOLTAGE) && (type != POWER) &&
                                  (type != WBOX0FIX) && (type != MBOX0TMP))) {
        fprintf(stderr, "ERROR: The selected register %s is in use.\n",
                counter_map[index].key);
        fprintf(stderr, "Please run likwid with force option (-f, --force) to "
                        "overwrite settings\n");
        type = NOTYPE;
      }
    }
...
static int getEvent(bstring event_str, bstring counter_str,
                    PerfmonEvent *event) {
...
static int assignOption(PerfmonEvent *event, bstring entry, int index,
                        EventOptionType type, int noval_value) {
  int found_double = -1;
...
static int parseOptions(struct bstrList *tokens, PerfmonEvent *event,
                        RegisterIndex index) {
    ...
    if (subtokens->qty == 1) {
      if (biseqcstr(subtokens->entry[0], "edgedetect") == 1) {
        event->numberOfOptions =
            assignOption(event, subtokens->entry[1], event->numberOfOptions,
                         EVENT_OPTION_EDGE, 1);
      } else if (biseqcstr(subtokens->entry[0], "invert") == 1) {
        event->numberOfOptions =
            assignOption(event, subtokens->entry[1], event->numberOfOptions,
                         EVENT_OPTION_INVERT, 1);
      } else if (biseqcstr(subtokens->entry[0], "kernel") == 1) {
        event->numberOfOptions =
            assignOption(event, subtokens->entry[1], event->numberOfOptions,
                         EVENT_OPTION_COUNT_KERNEL, 1);
...
static double calculateResult(int groupId, int eventId, int threadId) {
  ...
  if (counter->overflows == 0) {
    ...
    const double overflowPivotValue =
        ldexp(1.0, box_map[counter_map[event->index].type].regWidth);
    result += overflowPivotValue - (double)counter->startData +
              (double)counter->counterData;
...
void perfmon_check_counter_map(int cpu_id) {
  ...
  if (!HPMinitialized()) {
    ...
    if (HPMaddThread(cpu_id) != 0) {
      ERROR_PRINT(
          "Cannot check counters without access to performance counters");
      return;
    }
    own_hpm = 1;
  }
  ...
  for (int i = 0; i < perfmon_numCounters; i++) {
    ...
    if (cpuid_info.isIntel && counter_map[i].type == PMC &&
        (counter_map[i].index - counter_map[startpmcindex].index) >=
            cpuid_info.perf_num_ctr) {
      counter_map[i].type = NOTYPE;
      counter_map[i].optionMask = 0x0ULL;
    }
#ifndef LIKWID_USE_PERFEVENT
    DEBUG_PRINT(DEBUGLEV_DEVELOP, "Counter %s at pos %d with dev %s (%d) %d",
                counter_map[i].key, i, pci_device_names[counter_map[i].device],
                counter_map[i].device, cpu_id);
    ...
    DEBUG_PRINT(DEBUGLEV_DEVELOP, "Counter %s at pos %d with dev %s (%d): %s",
                counter_map[i].key, i, pci_device_names[counter_map[i].device],
                counter_map[i].device, translate_types[type]);
  ...
  for (int i = 0; i < perfmon_numArchEvents; i++) {
    ...
    for (int j = 0; j < perfmon_numCounters; j++) {
      ...
      if (getEvent(estr, cstr, &event) &&
          checkCounter(cstr, eventHash[i].limit)) {
        found = 1;
        bdestroy(cstr);
        break;
      }
    ...
    if (!found) {
      DEBUG_PRINT(DEBUGLEV_DEVELOP,
                  ...
                  eventHash[i].limit, eventHash[i].name);
      eventHash[i].limit = "";
    }
  }
  maps_checked = 1;
...
typedef int (*arch_perfmon_init_counter_map)(int num_in_counters,
                                             RegisterMap *in_counters,
                                             int *num_out_counters,
                                             RegisterMap **out_counters);

int perfmon_init_maps(void) {
  int err = 0;
  arch_perfmon_init_counter_map post_counter_func = NULL;
  if (eventHash != NULL && counter_map != NULL && box_map != NULL &&
      perfmon_numCounters > 0 && perfmon_numArchEvents > 0) {
    // Already initialized
    return 0;
  }
  ...
  DEBUG_PRINT(DEBUGLEV_DEVELOP,
              ...
              cpuid_info.family, cpuid_info.model, cpuid_info.stepping);
#endif
#if defined(__ARM_ARCH_8A) || defined(__ARM_ARCH_7A__)
  DEBUG_PRINT(DEBUGLEV_DEVELOP,
              "Initialize maps for family 0x%X vendor 0x%X part 0x%X",
              cpuid_info.family, cpuid_info.family, cpuid_info.model);
#endif
  ...
  if (post_counter_func) {
    ...
    err = post_counter_func(perfmon_numCounters, counter_map, &num_counters,
                            &counters);
    if (err != 0) {
      errno = -err;
  ...
  if ((eventHash != NULL) && (err == 0)) {
    ...
    PerfmonEvent *tmp =
        malloc((perfmon_numArchEvents + 10) * sizeof(PerfmonEvent));
    if (tmp) {
      DEBUG_PRINT(DEBUGLEV_DEVELOP, "Copying %d events from input list",
      ...
      eventHash[perfmon_numArchEvents].options[0].type =
          EVENT_OPTION_GENERIC_CONFIG;
      eventHash[perfmon_numArchEvents].options[0].value = 0x0ULL;
      eventHash[perfmon_numArchEvents].options[1].type =
          EVENT_OPTION_GENERIC_UMASK;
...
int perfmon_init_funcs(int *init_power, int *init_temp) {
  ...
  case P6_FAMILY:
    ...
    case ATOM_SILVERMONT_AIR:
      ...
      perfmon_finalizeCountersThread =
          perfmon_finalizeCountersThread_silvermont;
      break;
    ...
    case WESTMERE_EX:
      ...
      perfmon_finalizeCountersThread =
          perfmon_finalizeCountersThread_westmereEX;
      break;
    ...
    case SANDYBRIDGE:
      ...
      perfmon_finalizeCountersThread =
          perfmon_finalizeCountersThread_sandybridge;
      break;
    ...
    case SAPPHIRERAPIDS:
      ...
      perfmon_finalizeCountersThread =
          perfmon_finalizeCountersThread_sapphirerapids;
      break;
    ...
    case EMERALDRAPIDS:
      ...
      perfmon_finalizeCountersThread =
          perfmon_finalizeCountersThread_emeraldrapids;
      break;
    ...
    case SIERRAFORREST:
      ...
      perfmon_finalizeCountersThread =
          perfmon_finalizeCountersThread_sierraforrest;
      break;
...
int perfmon_init(int nrThreads, const int *threadsToCpu) {
  ...
  if (nrThreads <= 0) {
    errno = EINVAL;
    ERROR_PRINT("Number of threads must be greater than 0 but only %d given",
                nrThreads);
    return -errno;
  }
  ...
  if ((cpuid_info.family == 0) && (cpuid_info.model == 0)) {
    errno = ENODEV;
    ERROR_PRINT(
        "Topology module not inialized. Needed to determine current CPU type");
    return -errno;
  }
  ...
  groupSet->threads =
      (PerfmonThread *)malloc(nrThreads * sizeof(PerfmonThread));
  ...
  if (ret < 0) {
    errno = -ret;
    ERROR_PRINT("Failed to initialize event and counter lists for %s",
                cpuid_info.name);
    free(groupSet->threads);
    free(groupSet);
    groupSet = NULL;
...
int perfmon_addEventSet(const char *eventCString) {
  ...
  if (eventCString == NULL) {
    DEBUG_PRINT(
        DEBUGLEV_INFO,
        "Event string is empty. Trying environment variable LIKWID_EVENTS");
    eventCString = getenv("LIKWID_EVENTS");
    if (eventCString == NULL) {
      errno = EINVAL;
      ERROR_PRINT("Cannot read event string. Also event string from "
  ...
  DEBUG_PRINT(DEBUGLEV_INFO, "Currently %d groups of %d active",
              groupSet->numberOfActiveGroups + 1, groupSet->numberOfGroups + 1);
  ...
  if (perf_pid != NULL) {
#ifdef LIKWID_USE_PERFEVENT
    snprintf(cstringcopy, strlen(eventCString) - strlen(perf_pid), "%s",
             eventCString);
#endif
  }

  if (strchr(cstringcopy, ':') == NULL) {
    err = perfgroup_readGroup(
        config->groupPath, cpuid_info.short_name, cstringcopy,
    ...
    if (err == -EACCES) {
      ...
      errno = ENODEV;
      ERROR_PRINT(
          "Performance group %s only available with deactivated HyperThreading",
          eventCString);
      return err;
    } else if (err < 0) {
      errno = -err;
  ...
  char *evstr = perfgroup_getEventStr(
      &groupSet->groups[groupSet->numberOfActiveGroups].group);
  if (perf_pid != NULL) {
  ...
  eventSet->events = (PerfmonEventSetEntry *)malloc(
      eventtokens->qty * sizeof(PerfmonEventSetEntry));
  ...
  for (i = 0; i < eventtokens->qty; i++) {
    ...
    if (subtokens->qty < 2) {
...
#ifndef LIKWID_USE_PERFEVENT
      event->type = checkAccess(subtokens->entry[1], event->index, event->type,
                                forceOverwrite);
      if (event->type == NOTYPE) {
        DEBUG_PRINT(DEBUGLEV_INFO, "Cannot access counter register %s",
                    bdata(subtokens->entry[1]));
        event->type = NOTYPE;
        goto past_checks;
      }
      ...
      if (cpuid_info.vendor == APPLE_M1 &&
          cpuid_info.model == APPLE_M1_STUDIO) {
        type = FPMC;
      }
      if (!translate_types[type]) {
        DEBUG_PRINT(DEBUGLEV_INFO, "Cannot access counter register %s",
    ...
    past_checks:
      ...
      if (event->type != NOTYPE) {
        valid_events++;
        DEBUG_PRINT(DEBUGLEV_INFO, "Added event %s for counter %s to group %d",
...
int perfmon_setupCounters(int groupId) {
  ...
  for (i = 0; i < groupSet->numberOfThreads; i++) {
    if (force_setup) {
      memset(currentConfig[groupSet->threads[i].processorId], 0,
             NUM_PMC * sizeof(uint64_t));
    }
    ret =
        __perfmon_setupCountersThread(groupSet->threads[i].thread_id, groupId);
    if (ret != 0) {
      return ret;
    }
  }
...
int __perfmon_startCounters(int groupId) {
  ...
  for (; i < groupSet->numberOfThreads; i++) {
    ...
    ret = perfmon_startCountersThread(groupSet->threads[i].thread_id,
                                      &groupSet->groups[groupId]);
    if (ret) {
...
int __perfmon_stopCounters(int groupId) {
  ...
  for (i = 0; i < groupSet->numberOfThreads; i++) {
    ret = perfmon_stopCountersThread(groupSet->threads[i].thread_id,
                                     &groupSet->groups[groupId]);
    if (ret) {
      return -groupSet->threads[i].thread_id - 1;
    }
...
int __perfmon_readCounters(int groupId, int threadId) {
  ...
  if (threadId == -1) {
      ...
      for (j = 0; j < groupSet->groups[groupId].numberOfEvents; j++) {
        if (groupSet->groups[groupId].events[j].type != NOTYPE) {
          ...
          groupSet->groups[groupId]
              .events[j]
              .threadCounter[threadId]
              .startData = groupSet->groups[groupId]
                               .events[j]
                               .threadCounter[threadId]
                               .counterData;
        }
    ...
    for (j = 0; j < groupSet->groups[groupId].numberOfEvents; j++) {
      result = (double)calculateResult(groupId, j, threadId);
      groupSet->groups[groupId].events[j].threadCounter[threadId].lastResult =
          result;
...
double perfmon_getResult(int groupId, int eventId, int threadId) {
  ...
  if ((groupSet->groups[groupId]
    ...
    return groupSet->groups[groupId]
        .events[eventId]
        .threadCounter[threadId]
        .lastResult;
  }
...
double perfmon_getMetric(int groupId, int metricId, int threadId) {
  ...
  if ((metricId < 0) ||
      (metricId >= groupSet->groups[groupId].group.nmetrics)) {
    return NAN;
  }
  timer_init();
  init_clist(&clist);
  ...
  if (cpu != sock_cpu) {
    ...
    for (e = 0; e < groupSet->groups[groupId].numberOfEvents; e++) {
      if (perfmon_isUncoreCounter(
        ...
        err = update_clist(&clist, groupSet->groups[groupId].group.counters[e],
                           perfmon_getResult(groupId, e, sock_cpu));
        if (err < 0) {
          DEBUG_PRINT(DEBUGLEV_DEVELOP,
                      "Cannot add socket result of counter %s for thread %d",
                      groupSet->groups[groupId].group.counters[e], threadId);
        }
      }
    }
  }
  e = calc_metric(groupSet->groups[groupId].group.metricformulas[metricId],
                  &clist, &result);
  if (e < 0) {
    result = 0.0;
...
int __perfmon_switchActiveGroupThread(int thread_id, int new_group) {
  ...
  if (state == STATE_SETUP) {
    for (i = 0; i < groupSet->groups[groupSet->activeGroup].numberOfEvents;
         i++) {
...
int perfmon_switchActiveGroup(int new_group) {
  ...
  int ret = 0;
  for (i = 0; i < groupSet->numberOfThreads; i++) {
    ret = __perfmon_switchActiveGroupThread(groupSet->threads[i].thread_id,
                                            new_group);
    if (ret != 0) {
      return ret;
    }
...
int perfmon_getGroups(char ***groups, char ***shortinfos, char ***longinfos) {
  ...
  ret = perfgroup_getGroups(config->groupPath, cpuid_info.short_name, groups,
                            shortinfos, longinfos);
  return ret;
}

void perfmon_returnGroups(int nrgroups, char **groups, char **shortinfos,
                          char **longinfos) {
...
double perfmon_getMetricOfRegionThread(int region, int metricId, int threadId) {
  ...
  if (metricId < 0 ||
      metricId >=
          groupSet->groups[markerResults[region].groupID].group.nmetrics) {
    return NAN;
  }
  timer_init();
  init_clist(&clist);
  for (e = 0; e < markerResults[region].eventCount; e++) {
    ...
    if (err) {
      printf(
          "Cannot add counter %s to counter list for metric calculation\n",
          counter_map
              [groupSet->groups[markerResults[region].groupID].events[e].index]
                  .key);
      destroy_clist(&clist);
      return 0;
    }
  ...
  if (cpu != sock_cpu) {
    ...
    for (e = 0; e < markerResults[region].eventCount; e++) {
      if (perfmon_isUncoreCounter(
              groupSet->groups[markerResults[region].groupID]
                  .group.counters[e]) &&
          !perfmon_isUncoreCounter(
              groupSet->groups[markerResults[region].groupID]
  ...
  err = calc_metric(groupSet->groups[markerResults[region].groupID]
                        .group.metricformulas[metricId],
                    &clist, &result);
  if (err < 0) {
    ERROR_PRINT("Cannot calculate formula %s",
    ...
    if (strchr(buf, ':')) {
      ...
      *ptr = '\0';
      ...
      if (cpu >= 0) {
        ...
        markerResults[regionid].counters[cpuidx] =
            lw_calloc(nevents, sizeof(double));

        eventidx = 0;
        ptr = strtok(remain, " ");
        while (ptr != NULL && eventidx < nevents) {
          sscanf(ptr, "%lf",
                 &(markerResults[regionid].counters[cpuidx][eventidx]));
          ptr = strtok(NULL, " ");
          eventidx++;
        }
        regionCPUs[regionid]++;
