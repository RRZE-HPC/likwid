...
parseOptions(struct bstrList *tokens, PerfmonEvent *event, RegisterIndex index)
{
    ...
    for (i = 0; i < event->numberOfOptions; i++) {
        if (event->options[i].type == EVENT_OPTION_EDGE) {
            ...
            if ((threshold_set == FALSE) && (event->numberOfOptions < MAX_EVENT_OPTIONS) && (edge_set == TRUE || invert_set == TRUE)) {
                event->options[event->numberOfOptions].type = EVENT_OPTION_THRESHOLD;
                event->options[event->numberOfOptions].value = 0x1;
                event->numberOfOptions++;
            } else {
...
calculateResult(int groupId, int eventId, int threadId)
{
    ...
    } else if ((counter_map[event->index].type == THERMAL) || (counter_map[event->index].type == MBOX0TMP)) {
...
void perfmon_check_counter_map(int cpu_id)
{
    ...
    for (int i = 0; i < perfmon_numCounters; i++) {
        ...
        if (cpuid_info.isIntel && counter_map[i].type == PMC && (counter_map[i].index - counter_map[startpmcindex].index) >= cpuid_info.perf_num_ctr) {
...
int perfmon_init(int nrThreads, const int *threadsToCpu)
{
    ...
    for (i = 0; i < cpuid_topology.numSockets; i++)
...
int perfmon_addEventSet(const char *eventCString)
{
    ...
    if (((valid_events > fixed_counters) || isPerfGroup) && ((eventSet->regTypeMask1 != 0x0ULL) || (eventSet->regTypeMask2 != 0x0ULL) || (eventSet->regTypeMask3 != 0x0ULL) || (eventSet->regTypeMask4 != 0x0ULL) || (eventSet->regTypeMask5 != 0x0ULL) || (eventSet->regTypeMask6 != 0x0ULL) || (eventSet->regTypeMask7 != 0x0ULL) || (eventSet->regTypeMask8 != 0x0ULL) || (eventSet->regTypeMask9 != 0x0ULL))) {
        ...
        for (threadId = 0; threadId < groupSet->numberOfThreads; threadId++) {
            ...
            for (j = 0; j < groupSet->groups[groupId].numberOfEvents; j++) {
                if (groupSet->groups[groupId].events[j].type != NOTYPE) {
                    ...
                    groupSet->groups[groupId].events[j].threadCounter[threadId].startData = groupSet->groups[groupId].events[j].threadCounter[threadId].counterData;
                }
            }
        }
...
perfmon_getResult(int groupId, int eventId, int threadId)
{
    ...
    if ((groupSet->groups[groupId].events[eventId].threadCounter[threadId].fullResult == 0) || (groupSet->groups[groupId].events[eventId].type == THERMAL) || (groupSet->groups[groupId].events[eventId].type == VOLTAGE) || (groupSet->groups[groupId].events[eventId].type == MBOX0TMP) || (groupSet->groups[groupId].events[eventId].type == QBOX0FIX) || (groupSet->groups[groupId].events[eventId].type == QBOX1FIX) || (groupSet->groups[groupId].events[eventId].type == QBOX2FIX) || (groupSet->groups[groupId].events[eventId].type == SBOX0FIX) || (groupSet->groups[groupId].events[eventId].type == SBOX1FIX) || (groupSet->groups[groupId].events[eventId].type == SBOX2FIX)) {
...
perfmon_getMetric(int groupId, int metricId, int threadId)
{
    ...
    if (cpu != sock_cpu) {
        ...
        for (e = 0; e < groupSet->groups[groupId].numberOfEvents; e++) {
            if (perfmon_isUncoreCounter(groupSet->groups[groupId].group.counters[e]) && !perfmon_isUncoreCounter(groupSet->groups[groupId].group.metricformulas[metricId])) {
...
perfmon_getEventName(int groupId, int eventId)
{
    ...
    if ((groupSet->groups[groupId].group.nevents == 0) || (eventId > groupSet->groups[groupId].group.nevents)) {
        return NULL;
    }
...
perfmon_getMetricOfRegionThread(int region, int metricId, int threadId)
{
    ...
    if (cpu != sock_cpu) {
        ...
        for (e = 0; e < markerResults[region].eventCount; e++) {
            if (perfmon_isUncoreCounter(groupSet->groups[markerResults[region].groupID].group.counters[e]) && !perfmon_isUncoreCounter(groupSet->groups[markerResults[region].groupID].group.metricformulas[metricId])) {
