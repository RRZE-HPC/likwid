...
intelCpuidFunc_4(CacheLevel **cachePool)
{
    ...
    *cachePool = (CacheLevel *)malloc(maxNumLevels * sizeof(CacheLevel));
    ...
    for (i = 0; i < maxNumLevels; i++) {
        ...
        if (pool[i].threads > (int)(cpuid_topology.numCoresPerSocket * cpuid_topology.numThreadsPerCore)) {
        ...
        case P6_FAMILY:
            ...
            for (uint32_t i = 0; i < cpuid_topology.numHWThreads; i++) {
                ...
                /* ThreadId is extracted from th apicId using the bit width
                     ...
                     * */
                hwThreadPool[id].threadId = extractBitField(hwThreadPool[id].apicId,
        ...
        case K8_FAMILY:
            ...
            for (uint32_t i = 0; i < cpuid_topology.numHWThreads; i++) {
                ...
                /* ThreadId is extracted from th apicId using the bit width
                     * of the number of logical processors
                     * */
                hwThreadPool[id].threadId = extractBitField(hwThreadPool[i].apicId,
                    getBitFieldWidth(maxNumLogicalProcsPerCore), 0);
                ...
                /* CoreId is extracted from th apicId using the bitWidth
                     ...
                     * bit width of the number of cores as width
                     * */
                hwThreadPool[id].coreId = extractBitField(hwThreadPool[i].apicId,
                    getBitFieldWidth(maxNumCores),
                    0);
    ...
    case K15_FAMILY:
        ...
        while (type) {
            ...
            if ((type == DATACACHE) || (type == UNIFIEDCACHE)) {
                ...
                cachePool[maxNumLevels].size = cachePool[maxNumLevels].associativity * cachePool[maxNumLevels].lineSize * cachePool[maxNumLevels].sets;
