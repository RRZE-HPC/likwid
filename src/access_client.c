...
static char *access_client_strerror(AccessErrorType det)
{
    switch (det) {
    case ERR_NOERROR:
        return "No error";
...
static void access_client_signal_catcher(int sig) { close_access_client(); }

static int access_client_catch_signal()
{
...
static int access_client_startDaemon_direct(int cpu_id, struct sockaddr_un *address)
{
...
static int access_client_startDaemon_bridge(
    int cpu_id, const char *bridge_path, struct sockaddr_un *daemon_address)
{
...
static int access_client_daemon_connect(int cpu_id, struct sockaddr_un *address)
{
    int res = 0;
    char *filepath;
...
int access_client_check(PciDeviceIndex dev, int cpu_id)
{
    ...
    if ((cpuSockets[cpu_id] > 0) || ((cpuSockets_open == 1) && (globalSocket > 0))) {
        ...
        if (record.errorcode == ERR_NOERROR) {
            ...
            if (dev == MSR_DEV) {
                DEBUG_PRINT(DEBUGLEV_DEVELOP,
                    "Device check for dev %d on CPU %d with accessDaemon failed",
                    dev,
                    record.cpu,
                    access_client_strerror(record.errorcode));
            } else {
                DEBUG_PRINT(DEBUGLEV_DEVELOP,
                    "Device check for dev %d on socket %d with accessDaemon failed",
                    dev,
                    record.cpu,
                    access_client_strerror(record.errorcode));
            }
