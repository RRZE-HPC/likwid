...
static int cpufreq_amd_pstate_lowest_nonlinear_freq_getter(
    const LikwidDevice_t device, char **value)
{
...
int likwid_sysft_init_cpufreq(_SysFeatureList *out)
{
    ...
    if (cpufreq_intel_pstate_test()) {
        ...
        DEBUG_PRINT(
            DEBUGLEV_INFO, "Registering AMD pstate (amd-pstate=active) cpufreq knobs for cpufreq");
        ...
        DEBUG_PRINT(DEBUGLEV_INFO,
            "Registering AMD pstate (amd-pstate=passive/guided) cpufreq knobs for cpufreq");
