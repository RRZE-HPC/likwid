...
#define DLSYM_AND_CHECK(dllib, name) name##_ptr = dlsym(dllib, #name);  \
    do {                                                                \
        ...
        if (err_) {                                                     \
            return ERROR_SET("Error: dlsym on symbol '%s' failed with error: %s", #name, err_);\
        }\
    } while (0)
...
#define NVML_CALL(func, ...)                            \
    do {                                                \
        ...
        if (s_ != NVML_SUCCESS) {                        \
            return ERROR_SET_NVML(s_, "NVML function %s failed", #func);\
        }                                               \
    } while (0)
...
__attribute__((destructor)) static void cleanup_nvml(void)
{
    if (!nvml_initialized)
        return;
...
cerr_t likwid_sysft_init_nvml(_SysFeatureList *list)
{
    ...
    if (!dl_nvml) {
        PRINT_INFO("Cannot load libnvidia-ml.so: %s", dlerror());
        PRINT_INFO("Sysfeatures NVML support not available");
        return NULL;
    }
    ...
    if (nverr != NVML_SUCCESS) {
        dlclose(dl_nvml);
        return ERROR_SET_NVML(nverr, "nvmlInit_v2 failed");
    }
...
{
    assert(lwDevice->type == DEVICE_TYPE_NVIDIA_GPU);
...
static cerr_t nvidia_gpu_devices_available_getter(const LikwidDevice_t device, char **value)
{
    assert(device->type == DEVICE_TYPE_NODE);
...
static cerr_t nvidia_gpu_clock_info_getter(const LikwidDevice_t device, nvmlClockType_t clockType, nvmlClockId_t clockId, char **value)
{
...
static cerr_t nvidia_gpu_ecc_error_getter(const LikwidDevice_t device, EccErrType type, EccErrMem mem, char **value)
{
    LWD_TO_NVMLD(device, nvmlDevice);
...
{
    uint64_t watts;
    if (likwid_sysft_string_to_uint64(value, &watts))
        return ERROR_SET("likwid_sysft_string_to_uint64 failed");
...
static cerr_t nvidia_gpu_power_limit_minmax_getter(const LikwidDevice_t device, bool max, char **value)
{
    LWD_TO_NVMLD(device, nvmlDevice);
...
static cerr_t nvidia_gpu_encoder_usage_getter(const LikwidDevice_t device, char **value)
{
    LWD_TO_NVMLD(device, nvmlDevice);
...
static cerr_t nvidia_gpu_gom_getter(const LikwidDevice_t device, bool usePending, char **value)
{
    LWD_TO_NVMLD(device, nvmlDevice);
...
static cerr_t nvidia_gpu_gom_setter(const LikwidDevice_t device, const char *value)
{
    ...
    return ERROR_SET("Cannot set GPU operation mode to '%s', which is not valid", value);
}
...
static cerr_t nvidia_gpu_temp_thresh_setter(const LikwidDevice_t device, nvmlTemperatureThresholds_t t, const char *value)
{
    uint64_t temp;
    if (likwid_sysft_string_to_uint64(value, &temp))
        return ERROR_SET("likwid_sysft_string_to_uint64 failed");
    if (temp > INT_MAX)
        temp = INT_MAX;
    int arg = (int)temp;
...
static cerr_t nvidia_gpu_ofa_usage_getter(const LikwidDevice_t device, char **value)
{
    LWD_TO_NVMLD(device, nvmlDevice);
