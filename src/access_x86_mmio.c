...
#define FORM_PCI_ADDR(bus, dev, fun, off)                                                          \
    (((PCI_ENABLE)) | ((bus & 0xFF) << 16) | ((dev & 0x1F) << 11) | ((fun & 0x07) << 8) |          \
        ((off & 0xFF) << 0))
...
static int mmio_validDevice(uint32_t pci_bus, uint32_t deviceId)
{
    ...
    if (pci_dev != deviceId) {
        ERROR_PRINT(
            "Cannot get device id: device ids do not match 0x%X and 0x%X", pci_dev, deviceId);
        bdestroy(bdevfile);
        return 0;
    }
    bdestroy(bdevfile);
...
static int mmio_fillBox(MMIOConfig *config, uint32_t pci_bus, int imc_idx, MMIOBoxHandle *handle)
{
    uint32_t tmp     = 0;
    off_t addr       = 0;
    off_t mem_offset = 0;
...
#pragma GCC diagnostic ignored "-Wnonnull"
    ...
    void *maddr = mmap(NULL,
        config->channel_count * ICX_IMC_MMIO_SIZE,
...
static int mmio_fillFreerunBox(
    MMIOConfig *config, uint32_t pci_bus, int imc_idx, MMIOBoxHandle *handle)
{
    uint32_t tmp     = 0;
    off_t addr       = 0;
    off_t mem_offset = 0;
    ...
    *data       = d;
    ...
    if (box) {
        ...
        DEBUG_PRINT(DEBUGLEV_DEVELOP,
            "Read MMIO counter 0x%X Dev %d on socket %d: 0x%lX",
            reg,
            imc_idx,
            socket,
            d);
...
int access_x86_mmio_write(PciDeviceIndex dev, const int socket, uint32_t reg, uint64_t data)
{
    ...
    if (sbox) {
        ...
        if (box) {
            DEBUG_PRINT(DEBUGLEV_DEVELOP,
                "Write MMIO counter 0x%X Dev %d on socket %d: 0x%lX",
                reg,
                imc_idx,
                socket,
                data);
