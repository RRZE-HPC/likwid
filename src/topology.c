...
static int readTopologyFile(const char *filename, cpu_set_t cpuSet)
{
    ...
    while (fgets(line, 512, fp) != NULL) {
        ...
        if ((strncmp(structure, "cpuid_topology", 14) == 0) &&
            (strncmp(field, "numHWThreads", 12) == 0)) {
            sscanf(line, "%s %s = %d", structure, field, &numHWThreads);
        } else if ((strncmp(structure, "cpuid_topology", 14) == 0) &&
                   (strncmp(field, "numCacheLevels", 15) == 0)) {
            sscanf(line, "%s %s = %d", structure, field, &numCacheLevels);
        } else if ((strncmp(structure, "numa_info", 9) == 0) &&
                   (strncmp(field, "numberOfNodes", 13) == 0)) {
    ...
    while (fgets(line, 512, fp) != NULL) {
        ...
        if ((strncmp(structure, "numa_info", 9) == 0) &&
            (strncmp(value, "numberOfProcessors", 19) == 0)) {
    ...
    for (i = 0; i < numberOfNodes; i++) {
        ...
        if (!numa_info.nodes[i].processors) {
            for (int j = 0; j < i; j++) {
                if (numa_info.nodes[j].processors)
                    free(numa_info.nodes[j].processors);
                if (numa_info.nodes[j].distances)
                    free(numa_info.nodes[j].distances);
            }
            free(tmpNumberOfProcessors);
            free(cpuid_topology.cacheLevels);
            free(cpuid_topology.threadPool);
