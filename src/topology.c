...
static const char *short_zen5  = "zen5";
...
static void initTopologyFile(FILE *file)
{
    ...
    if (fread(hwThreadPool, sizeof(*hwThreadPool), cpuid_topology.numHWThreads, file) !=
        sizeof(sizeof(*hwThreadPool))) {
    ...
    if (fread(cacheLevels, sizeof(*cacheLevels), cpuid_topology.numCacheLevels, file) !=
        sizeof(*cacheLevels)) {
...
static int readTopologyFile(const char *filename, cpu_set_t cpuSet)
{
    ...
    numProcessorsPerNumaNode =
        lw_calloc(numa_info.numberOfNodes, sizeof(*numProcessorsPerNumaNode));
    int curNumaNode = 0;
    while (fgets(line, sizeof(line), fp) != NULL) {
