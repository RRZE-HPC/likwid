...
int likwid_sysft_add_to_feature_list(LikwidSysFeatureList *list, const LikwidSysFeature *feature)
{
    ...
    LikwidSysFeature *flist =
        realloc(list->features, (list->num_features + 1) * sizeof(LikwidSysFeature));
    if (!flist) {
...
int likwid_sysft_merge_feature_lists(LikwidSysFeatureList *inout, const LikwidSysFeatureList *in)
{
    ...
    LikwidSysFeature *flist = realloc(
        inout->features, (inout->num_features + in->num_features) * sizeof(LikwidSysFeature));
    if (!flist) {
        ERROR_PRINT("Cannot allocate space for extended feature list");
        return -ENOMEM;
    }
...
void likwid_sysft_free_feature_list(LikwidSysFeatureList *list)
{
    ...
    assert(list->num_features >= 0);
    for (size_t i = 0; i < (size_t)list->num_features; i++) {
...
int _merge_feature_lists(_SysFeatureList *inout, const _SysFeatureList *in)
{
    ...
    _SysFeature *flist =
        realloc(inout->features, (inout->num_features + in->num_features) * sizeof(_SysFeature));
    if (!flist) {
        ERROR_PRINT("Cannot allocate space for extended feature list");
        return -ENOMEM;
    }
...
int likwid_sysft_internal_to_external_feature_list(
    const _SysFeatureList *inlist, LikwidSysFeatureList *outlist)
{
    if (!inlist || !outlist) {
    ...
    assert(inlist->num_features >= 0);
    for (size_t i = 0; i < (size_t)inlist->num_features; i++) {
        ...
        out->category = strdup(in->category);
        if (!out->category) {
            err = -errno;
            goto cleanup;
        }
...
cleanup:
    likwid_sysft_free_feature_list(outlist);
    return err;
}
...
void likwid_sysft_printlistint(const _SysFeatureList *list)
{
    ...
    for (int i = 0; i < list->num_features; i++) {
        ...
        printf("[%03d] name=%s category=%s description=%s getter=%p setter=%p type=%d tester=%p "
               "unit=%s\n",
            i,
            f->name,
            cat,
