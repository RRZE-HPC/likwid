...
#define DECLARECUFUNC(funcname, funcsig)                                                           \
    ...
    CUresult(*funcname##TopoPtr) funcsig;
...
#define DECLARECUDAFUNC(funcname, funcsig)                                                         \
    ...
    cudaError_t(*funcname##TopoPtr) funcsig;
...
static int cuda_topo_link_libraries(void)
{
#define DLSYM_AND_CHECK(dllib, name)                                                               \
    ...
    cuDeviceComputeCapabilityTopoPtr =
        DLSYM_AND_CHECK(topo_dl_libcuda, "cuDeviceComputeCapability");
...
static int cuda_topo_init(void)
{
    CUresult cuErr = (*cuInitTopoPtr)(0);
    if (cuErr != CUDA_SUCCESS) {
        DEBUG_PRINT(
            DEBUGLEV_INFO, "CUDA cannot be found and initialized (cuInit failed): %d", cuErr);
        return -ENODEV;
    }
    return 0;
}

static int cuda_topo_get_numDevices(void)
{
    int count      = 0;
    ...
    if (cuErr == CUDA_SUCCESS)
        return count;
    DEBUG_PRINT(
        DEBUGLEV_INFO, "uDeviceGetCount failed even though cuda_topo_init succeeded: %d", cuErr);
    return -ELIBACC;
}

static int cuda_topo_get_numNode(int pci_bus, int pci_dev, int pci_domain)
{
    char fname[1024];
    char buff[100];
    int ret = snprintf(fname,
        1023,
        "/sys/bus/pci/devices/0000:%02x:%02x.%1x/numa_node",
        pci_bus,
        pci_dev,
        pci_domain);
    if (ret > 0) {
...
int topology_cuda_init()
{
    ...
    CUDA_CALL((*cudaDriverGetVersionTopoPtr)(&cuda_version), ret = -1;
        goto topology_gpu_init_error;);
    CUDA_CALL((*cudaRuntimeGetVersionTopoPtr)(&cudart_version), ret = -1;
        goto topology_gpu_init_error;);
    if (num_devs > 0) {
        ...
        for (i = 0; i < num_devs; i++) {
            ...
            if (cuda_version >= 10000 && cudart_version >= 10000) {
                CU_CALL((*cuDeviceTotalMem_v2TopoPtr)(&s, dev), ret = -ENOMEM;
                    goto topology_gpu_init_error;);
                if (s == 0) {
                    CU_CALL((*cuDeviceTotalMemTopoPtr)(&s, dev), ret = -ENOMEM;
                        goto topology_gpu_init_error;);
                }
            } else {
                CU_CALL((*cuDeviceTotalMemTopoPtr)(&s, dev), ret = -ENOMEM;
            ...
            CU_CALL((*cuDeviceGetNameTopoPtr)(cudaTopology.devices[i].name, NAME_LONG_MAX - 1, dev),
                ret = -ENOMEM;
                goto topology_gpu_init_error;);
            ...
            CU_CALL(
                (*cuDeviceComputeCapabilityTopoPtr)(
                    &cudaTopology.devices[i].ccapMajor, &cudaTopology.devices[i].ccapMinor, dev),
                ret = -ENOMEM;
                goto topology_gpu_init_error;);
            if (cudaTopology.devices[i].ccapMajor < 7) {
                ret = snprintf(
                    cudaTopology.devices[i].short_name, NAME_SHORT_MAX, "nvidia_gpu_cc_lt_7");
            } else if (cudaTopology.devices[i].ccapMajor >= 7) {
                ret = snprintf(
                    cudaTopology.devices[i].short_name, NAME_SHORT_MAX, "nvidia_gpu_cc_ge_7");
            }
            CUdevprop props;
            CU_CALL((*cuDeviceGetPropertiesTopoPtr)(&props, dev), ret = -ENOMEM;
                goto topology_gpu_init_error;);
            ...
            CU_CALL((*cuDeviceGetAttributeTopoPtr)(
                        &cudaTopology.devices[i].l2Size, CU_DEVICE_ATTRIBUTE_L2_CACHE_SIZE, dev),
                ret = -ENOMEM;
                goto topology_gpu_init_error;);
            CU_CALL((*cuDeviceGetAttributeTopoPtr)(&cudaTopology.devices[i].memClockRatekHz,
            ...
            CU_CALL((*cuDeviceGetAttributeTopoPtr)(&cudaTopology.devices[i].memClockRatekHz,
                ...
                goto topology_gpu_init_error;);
            CU_CALL((*cuDeviceGetAttributeTopoPtr)(
                        &cudaTopology.devices[i].pciBus, CU_DEVICE_ATTRIBUTE_PCI_BUS_ID, dev),
                ret = -ENOMEM;
                goto topology_gpu_init_error;);
            CU_CALL((*cuDeviceGetAttributeTopoPtr)(
                        &cudaTopology.devices[i].pciDev, CU_DEVICE_ATTRIBUTE_PCI_DEVICE_ID, dev),
                ret = -ENOMEM;
                goto topology_gpu_init_error;);
            CU_CALL((*cuDeviceGetAttributeTopoPtr)(
                        &cudaTopology.devices[i].pciDom, CU_DEVICE_ATTRIBUTE_PCI_DOMAIN_ID, dev),
                ret = -ENOMEM;
                goto topology_gpu_init_error;);
            // TODO: Get PCI function through nvmlDeviceGetPciInfo_v3, nvmlPciInfo_t->function
            cudaTopology.devices[i].pciFunc = 0;
            CU_CALL((*cuDeviceGetAttributeTopoPtr)(&cudaTopology.devices[i].maxBlockRegs,
                        CU_DEVICE_ATTRIBUTE_MAX_REGISTERS_PER_BLOCK,
                        dev),
                ret = -ENOMEM;
                goto topology_gpu_init_error;);
            CU_CALL((*cuDeviceGetAttributeTopoPtr)(&cudaTopology.devices[i].numMultiProcs,
                        CU_DEVICE_ATTRIBUTE_MULTIPROCESSOR_COUNT,
                        dev),
                ret = -ENOMEM;
                goto topology_gpu_init_error;);
            CU_CALL((*cuDeviceGetAttributeTopoPtr)(&cudaTopology.devices[i].maxThreadPerMultiProc,
                        CU_DEVICE_ATTRIBUTE_MAX_THREADS_PER_MULTIPROCESSOR,
                        dev),
                ret = -ENOMEM;
                goto topology_gpu_init_error;);
            CU_CALL((*cuDeviceGetAttributeTopoPtr)(&cudaTopology.devices[i].memBusWidth,
                        CU_DEVICE_ATTRIBUTE_GLOBAL_MEMORY_BUS_WIDTH,
                        dev),
                ret = -ENOMEM;
                goto topology_gpu_init_error;);
            CU_CALL((*cuDeviceGetAttributeTopoPtr)(&cudaTopology.devices[i].unifiedAddrSpace,
                        CU_DEVICE_ATTRIBUTE_UNIFIED_ADDRESSING,
                        dev),
                ret = -ENOMEM;
                goto topology_gpu_init_error;);
            CU_CALL((*cuDeviceGetAttributeTopoPtr)(
                        &cudaTopology.devices[i].ecc, CU_DEVICE_ATTRIBUTE_ECC_ENABLED, dev),
                ret = -ENOMEM;
                goto topology_gpu_init_error;);
            CU_CALL((*cuDeviceGetAttributeTopoPtr)(&cudaTopology.devices[i].asyncEngines,
                        CU_DEVICE_ATTRIBUTE_ASYNC_ENGINE_COUNT,
                        dev),
                ret = -ENOMEM;
                goto topology_gpu_init_error;);
            CU_CALL((*cuDeviceGetAttributeTopoPtr)(&cudaTopology.devices[i].mapHostMem,
                        CU_DEVICE_ATTRIBUTE_CAN_MAP_HOST_MEMORY,
                        dev),
                ret = -ENOMEM;
                goto topology_gpu_init_error;);
            CU_CALL((*cuDeviceGetAttributeTopoPtr)(
                        &cudaTopology.devices[i].integrated, CU_DEVICE_ATTRIBUTE_INTEGRATED, dev),
                ret = -ENOMEM;
                goto topology_gpu_init_error;);
            CU_CALL((*cuDeviceGetAttributeTopoPtr)(&cudaTopology.devices[i].surfaceAlign,
                        CU_DEVICE_ATTRIBUTE_SURFACE_ALIGNMENT,
                        dev),
                ret = -ENOMEM;
                goto topology_gpu_init_error;);

            cudaTopology.devices[i].numaNode = cuda_topo_get_numNode(cudaTopology.devices[i].pciBus,
                cudaTopology.devices[i].pciDev,
                cudaTopology.devices[i].pciDom);
        }
...
void topology_cuda_finalize(void)
{
    if (topology_cuda_initialized) {
...
CudaTopology_t get_cudaTopology(void)
{
    if (topology_cuda_initialized) {
        return &cudaTopology;
    }
