...
int topology_cuda_init()
{
    ...
    if (num_devs > 0) {
        ...
        for (i = 0; i < num_devs; i++) {
            ...
            CU_CALL(ret = -ENOMEM; goto topology_gpu_init_error,
                cuDeviceGetName,
                cudaTopology.devices[i].name,
                NAME_LONG_MAX - 1,
                dev);
            ...
            CU_CALL(ret = -ENOMEM; goto topology_gpu_init_error,
                ...
                &cudaTopology.devices[i].ccapMajor,
                &cudaTopology.devices[i].ccapMinor,
                dev);
            ...
            CUdevprop props;
            CU_CALL(ret = -ENOMEM;
                goto topology_gpu_init_error, cuDeviceGetProperties, &props, dev);
            ...
            CU_CALL(ret = -ENOMEM; goto topology_gpu_init_error,
                ...
                &cudaTopology.devices[i].l2Size,
                CU_DEVICE_ATTRIBUTE_L2_CACHE_SIZE,
                dev);
            CU_CALL(ret = -ENOMEM; goto topology_gpu_init_error,
                cuDeviceGetAttribute,
                &cudaTopology.devices[i].memClockRatekHz,
            ...
            CU_CALL(ret = -ENOMEM; goto topology_gpu_init_error,
                cuDeviceGetAttribute,
                &cudaTopology.devices[i].maxBlockRegs,
                CU_DEVICE_ATTRIBUTE_MAX_REGISTERS_PER_BLOCK,
                dev);
            CU_CALL(ret = -ENOMEM; goto topology_gpu_init_error,
                cuDeviceGetAttribute,
                &cudaTopology.devices[i].numMultiProcs,
                CU_DEVICE_ATTRIBUTE_MULTIPROCESSOR_COUNT,
                dev);
            CU_CALL(ret = -ENOMEM; goto topology_gpu_init_error,
                cuDeviceGetAttribute,
                &cudaTopology.devices[i].maxThreadPerMultiProc,
                CU_DEVICE_ATTRIBUTE_MAX_THREADS_PER_MULTIPROCESSOR,
                dev);
            CU_CALL(ret = -ENOMEM; goto topology_gpu_init_error,
                cuDeviceGetAttribute,
                &cudaTopology.devices[i].memBusWidth,
                CU_DEVICE_ATTRIBUTE_GLOBAL_MEMORY_BUS_WIDTH,
                dev);
            CU_CALL(ret = -ENOMEM; goto topology_gpu_init_error,
                cuDeviceGetAttribute,
                &cudaTopology.devices[i].unifiedAddrSpace,
                CU_DEVICE_ATTRIBUTE_UNIFIED_ADDRESSING,
                dev);
            CU_CALL(ret = -ENOMEM; goto topology_gpu_init_error,
                cuDeviceGetAttribute,
                &cudaTopology.devices[i].ecc,
                CU_DEVICE_ATTRIBUTE_ECC_ENABLED,
                dev);
            CU_CALL(ret = -ENOMEM; goto topology_gpu_init_error,
                cuDeviceGetAttribute,
                &cudaTopology.devices[i].asyncEngines,
                CU_DEVICE_ATTRIBUTE_ASYNC_ENGINE_COUNT,
                dev);
            CU_CALL(ret = -ENOMEM; goto topology_gpu_init_error,
                cuDeviceGetAttribute,
                &cudaTopology.devices[i].mapHostMem,
                CU_DEVICE_ATTRIBUTE_CAN_MAP_HOST_MEMORY,
                dev);
            CU_CALL(ret = -ENOMEM; goto topology_gpu_init_error,
                cuDeviceGetAttribute,
                &cudaTopology.devices[i].integrated,
                CU_DEVICE_ATTRIBUTE_INTEGRATED,
                dev);
            CU_CALL(ret = -ENOMEM; goto topology_gpu_init_error,
                cuDeviceGetAttribute,
                &cudaTopology.devices[i].surfaceAlign,
