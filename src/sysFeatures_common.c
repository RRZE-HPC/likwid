...
int likwid_sysft_register_features(_SysFeatureList *features, const _SysFeatureList *in)
{
    ...
    for (int i = 0; i < in->num_features; i++) {
        ...
        if (f->tester) {
            if (f->tester()) {
                DEBUG_PRINT(
                    DEBUGLEV_DEVELOP, "Running test for feature %s.%s", f->category, f->name);
                int err = _add_to_feature_list(features, f);
                if (err < 0) {
                    ERROR_PRINT(
                        "Failed to add HW feature %s.%s to feature list", f->category, f->name);
                }
            } else {
                DEBUG_PRINT(DEBUGLEV_DEVELOP,
                    "Test function for feature %s.%s failed",
                    f->category,
                    f->name);
            }
        } else {
...
int likwid_sysft_init_generic(const _HWArchFeatures *infeatures, _SysFeatureList *list)
{
    ...
    for (unsigned c = 0; infeatures[c].family >= 0 && infeatures[c].model >= 0; c++) {
        if ((unsigned)infeatures[c].family == cpuinfo->family &&
            (unsigned)infeatures[c].model == cpuinfo->model) {
            DEBUG_PRINT(DEBUGLEV_DEVELOP,
                "Using feature list for CPU family 0x%X and model 0x%X",
                cpuinfo->family,
                cpuinfo->model);
    ...
    if (!feature_list) {
        ...
        DEBUG_PRINT(DEBUGLEV_INFO,
            "No architectural sysFeatures for family 0x%X and model 0x%X",
            cpuinfo->family,
            cpuinfo->model);
        return -ENOTSUP;
    }
...
int likwid_sysft_string_to_uint64(const char *str, uint64_t *value)
{
    ...
    if (v == 0 && errno != 0) {
        DEBUG_PRINT(DEBUGLEV_DEVELOP,
            "Conversion of string '%s' to uint64_t failed: %s",
            str,
            strerror(errno));
        return -errno;
    }
...
int likwid_sysft_string_to_double(const char *str, double *value)
{
    ...
    if (!endptr) {
        DEBUG_PRINT(DEBUGLEV_DEVELOP,
            "Conversion of string '%s' to double failed: %s",
            str,
            strerror(errno));
        return -errno;
    }
...
int likwid_sysft_foreach_core_testmsr_cb(
    uint64_t reg, likwid_sysft_msr_test_func testFunc, void *cbData)
{
    int err = topology_init();
    if (err < 0)
        return err;
...
int likwid_sysft_foreach_hwt_testmsr_cb(
    uint64_t reg, likwid_sysft_msr_test_func testFunc, void *cbData)
{
    int err = topology_init();
    if (err < 0)
...
int likwid_sysft_foreach_socket_testmsr_cb(
    uint64_t reg, likwid_sysft_msr_test_func testFunc, void *cbData)
{
    int err = topology_init();
    if (err < 0)
...
int likwid_sysft_readmsr_field(
    const LikwidDevice_t device, uint64_t reg, int bitoffset, int width, uint64_t *value)
{
    uint64_t msrData;
...
int likwid_sysft_readmsr_bit_to_string(
    const LikwidDevice_t device, uint64_t reg, int bitoffset, bool invert, char **value)
{
    uint64_t field;
...
int likwid_sysft_readmsr_field_to_string(
    const LikwidDevice_t device, uint64_t reg, int bitoffset, int width, char **value)
{
    uint64_t field;
    int err = likwid_sysft_readmsr_field(device, reg, bitoffset, width, &field);
    if (err < 0)
        return err;
    return likwid_sysft_uint64_to_string(field, value);
}

int likwid_sysft_writemsr_field(
    const LikwidDevice_t device, uint64_t reg, int bitoffset, int width, uint64_t value)
{
    int err = HPMinit();
    if (err < 0)
