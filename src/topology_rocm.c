...
#define DECLAREHIPFUNC(funcname, ...)                                                              \
    hipError_t __attribute__((weak)) funcname(__VA_ARGS__);                                        \
    static hipError_t (*funcname##_ptr)(__VA_ARGS__);

DECLAREHIPFUNC(hipGetProcAddress, const char *symbol, void **pfn, int hipVersion, uint64_t flags,
...
static int topo_link_libraries(void)
{
    ...
    res = (*hipGetProcAddress_ptr)("hipGetDeviceProperties",
        (void **)&hipGetDevicePropertiesFunc,
        hipVersion,
        flags,
        &symbolStatus);
    if (res != hipSuccess) {
    ...
    res = (*hipGetProcAddress_ptr)(
        "hipGetDeviceCount", (void **)&hipGetDeviceCountFunc, hipVersion, flags, &symbolStatus);
    if (res != hipSuccess) {
...
static int topo_get_numDevices(void)
{
    int count      = 0;
...
static int topo_get_numNode(int pci_domain, int pci_bus, int pci_dev)
{
    char fname[1024];
    char buff[100];
    int ret = snprintf(fname,
        1023,
        "/sys/bus/pci/devices/%04x:%02x:%02x.%1x/numa_node",
        pci_domain,
        pci_bus,
        pci_dev,
        0);
...
static void topo_rocm_cleanup(int numDevices)
{
...
static int topo_gpu_init(RocmDevice *device, int deviceId)
{
    hipError_t err;
...
void topology_rocm_finalize(void)
{
    if (topo_rocm_initialized) {
