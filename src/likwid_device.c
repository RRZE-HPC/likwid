...
static int parse_pci_addr(
    const char *id, uint16_t *domain, uint8_t *bus, uint8_t *dev, uint8_t *func)
{
...
static int device_create_pci(LikwidDeviceType type, int id, uint16_t domain, uint8_t bus,
    uint8_t dev, uint8_t func, LikwidDevice_t *device)
{
...
static int device_create_nvgpu_by_index(int id, LikwidDevice_t *device)
{
    ...
    for (int i = 0; i < topo->numDevices; i++) {
        ...
        return device_create_pci(DEVICE_TYPE_NVIDIA_GPU,
            topo->devices[i].devid,
            topo->devices[i].pciDom,
            topo->devices[i].pciBus,
            topo->devices[i].pciDev,
            0,
            device);
    }
...
static int device_create_nvgpu_by_pciaddr(
    uint16_t dom, uint8_t bus, uint8_t dev, uint8_t func, LikwidDevice_t *device)
{
    CudaTopology_t topo = get_cudaTopology();
    ...
    for (int i = 0; i < topo->numDevices; i++) {
        ...
        return device_create_pci(
            DEVICE_TYPE_NVIDIA_GPU, topo->devices[i].devid, dom, bus, dev, func, device);
    }
...
static int device_create_amdgpu_by_index(int id, LikwidDevice_t *device)
{
    ...
    for (int i = 0; i < topo->numDevices; i++) {
        ...
        // TODO we do not have the function ID, but it should usually be 0
        return device_create_pci(DEVICE_TYPE_AMD_GPU,
            topo->devices[i].devid,
            topo->devices[i].pciDom,
            topo->devices[i].pciBus,
            topo->devices[i].pciDev,
...
static int device_create_amdgpu_by_pciaddr(
    uint16_t dom, uint8_t bus, uint8_t dev, uint8_t func, LikwidDevice_t *device)
{
    RocmTopology_t topo = get_rocmTopology();
    ...
    for (int i = 0; i < topo->numDevices; i++) {
        ...
        return device_create_pci(
            DEVICE_TYPE_AMD_GPU, topo->devices[i].devid, dom, bus, dev, func, device);
    }
...
int likwid_device_create_from_string(LikwidDeviceType type, const char *id, LikwidDevice_t *device)
{
    ...
    if (parse_pci_addr(id_token, &dom, &bus, &dev, &func) == 0) {
        ...
        default:
            err = -EINVAL;
            DEBUG_PRINT(
                DEBUGLEV_DEVELOP, "Unable to use PCI address to create device type: %d", type);
            break;
        }
        ...
        DEBUG_PRINT(
            DEBUGLEV_DEVELOP, "Unable to parse '%s' as valid PCI address or integer", id_token);
        err = -EINVAL;
    }
...
static int likwid_device_get_list(
    LikwidDeviceType type, char ***id_list, size_t *id_list_count, bool cpuset_only)
{
