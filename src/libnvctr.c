...
void nvmon_markerClose(void)
{
    ...
    if (file != NULL) {
        ...
        bstring thread_regs_grps = bformat("%d %d %d", numberOfGPUs, numberOfRegions, numberOfCudaGroups);
        ...
        for (int j = 0; j < numberOfRegions; j++) {
            ...
            for (int i = 0; i < numberOfGPUs; i++) {
                ...
                if (!ret) {
                    bstring l = bformat("%d %d %d %u %e %d ", j, results->groupID,
    ...
    for (int i = 0; i < nvmon_getNumberOfGPUs(); i++) {
       ...
       * LIKWID_NVMON_PERFWORKS_BACKEND)*/
            ...
            GPUDEBUG_PRINT(DEBUGLEV_DEVELOP, "STOP Device %d Event %d"
                                             ": %f - %f",
                i, j, end, results->StartPMcounters[j]);
        }
