...
void nvmon_markerInit(void)
{
    ...
    if ((eventStr != NULL) && (gpuStr != NULL) && (gpuFileStr != NULL) && likwid_cuda_init == 0) {
        ...
        fprintf(stderr,
            "Running without GPU Marker API. Activate GPU Marker API "
...
void nvmon_markerClose(void)
{
    ...
    if (markerfile == NULL) {
        fprintf(stderr,
            "Is the application executed with LIKWID wrapper? No file "
...
void nvmon_markerGetRegion(
    const char *regionTag, int *nr_gpus, int *nr_events, double **events, double *time, int *count)
{
    if (!likwid_cuda_init) {
        *nr_gpus   = 0;
        *nr_events = 0;
    ...
    if (nr_events != NULL && events != NULL && *nr_events > 0) {
        for (int i = 0; i < MIN(nvmon_getNumberOfGPUs(), *nr_gpus); i++) {
            ...
            int ret = get_smap_by_key(cuda_maps[i], bdata(tag), (void **)&results);
            if (ret == 0) {
                for (int j = 0; j < MIN(nvmon_getNumberOfEvents(activeCudaGroup), *nr_events);
                    j++) {
...
int nvmon_markerResetRegion(const char *regionTag)
{
    ...
    for (int i = 0; i < nvmon_getNumberOfGPUs(); i++) {
        ...
        memset(results->PMcounters, 0, nvmon_getNumberOfEvents(activeCudaGroup) * sizeof(double));
        results->count = 0;
        results->time  = 0;
...
void nvmon_markerNextGroup(void)
{
    ...
    if (next_group != activeCudaGroup) {
        DEBUG_PRINT(
            DEBUGLEV_DEVELOP, "Switch from GPU group %d to group %d", activeCudaGroup, next_group);
