...
static int intel_rapl_energy_status_getter(
    const LikwidDevice_t device, char **value, uint32_t reg, const RaplDomainInfo *info)
{
...
static int intel_rapl_energy_limit_1_enable_getter(
    const LikwidDevice_t device, char **value, uint32_t reg, const RaplDomainInfo *info)
{
    int err = getset_unusedinfo_check(device, value, info, DEVICE_TYPE_SOCKET);
    if (err < 0)
...
static int intel_rapl_energy_limit_1_enable_setter(
    const LikwidDevice_t device, const char *value, uint32_t reg, const RaplDomainInfo *info)
{
    int err = getset_unusedinfo_check(device, value, info, DEVICE_TYPE_SOCKET);
    if (err < 0)
        return err;
...
static int intel_rapl_energy_limit_1_clamp_getter(
    const LikwidDevice_t device, char **value, uint32_t reg, const RaplDomainInfo *info)
{
    int err = getset_unusedinfo_check(device, value, info, DEVICE_TYPE_SOCKET);
    if (err < 0)
...
static int intel_rapl_energy_limit_1_clamp_setter(
    const LikwidDevice_t device, const char *value, uint32_t reg, const RaplDomainInfo *info)
{
    int err = getset_unusedinfo_check(device, value, info, DEVICE_TYPE_SOCKET);
    if (err < 0)
...
static int intel_rapl_energy_limit_1_getter(
    const LikwidDevice_t device, char **value, uint32_t reg, const RaplDomainInfo *info)
{
    int err = getset_unusedinfo_check(device, value, info, DEVICE_TYPE_SOCKET);
    if (err < 0)
...
static int intel_rapl_energy_limit_1_setter(
    const LikwidDevice_t device, const char *value, uint32_t reg, const RaplDomainInfo *info)
{
...
static int intel_rapl_energy_limit_1_time_getter(
    const LikwidDevice_t device, char **value, uint32_t reg, const RaplDomainInfo *info)
{
    int err = getset_unusedinfo_check(device, value, info, DEVICE_TYPE_SOCKET);
    if (err < 0)
...
static int intel_rapl_energy_limit_1_time_setter(
    const LikwidDevice_t device, const char *value, uint32_t reg, const RaplDomainInfo *info)
{
    int err = getset_unusedinfo_check(device, value, info, DEVICE_TYPE_SOCKET);
    if (err < 0)
...
static int intel_rapl_energy_limit_2_getter(
    const LikwidDevice_t device, char **value, uint32_t reg, const RaplDomainInfo *info)
{
    int err = getset_unusedinfo_check(device, value, info, DEVICE_TYPE_SOCKET);
    if (err < 0)
...
static int intel_rapl_energy_limit_2_setter(
    const LikwidDevice_t device, const char *value, uint32_t reg, const RaplDomainInfo *info)
{
    int err = getset_info_check(device, value, info, DEVICE_TYPE_SOCKET);
    if (err < 0)
...
static int intel_rapl_energy_limit_2_time_getter(
    const LikwidDevice_t device, char **value, uint32_t reg, const RaplDomainInfo *info)
{
    int err = getset_unusedinfo_check(device, value, info, DEVICE_TYPE_SOCKET);
    if (err < 0)
...
static int intel_rapl_energy_limit_2_time_setter(
    const LikwidDevice_t device, const char *value, uint32_t reg, const RaplDomainInfo *info)
{
    int err = getset_info_check(device, value, info, DEVICE_TYPE_SOCKET);
    if (err < 0)
...
static int intel_rapl_energy_limit_2_enable_getter(
    const LikwidDevice_t device, char **value, uint32_t reg, const RaplDomainInfo *info)
{
    int err = getset_unusedinfo_check(device, value, info, DEVICE_TYPE_SOCKET);
    if (err < 0)
...
static int intel_rapl_energy_limit_2_enable_setter(
    const LikwidDevice_t device, const char *value, uint32_t reg, const RaplDomainInfo *info)
{
    int err = getset_unusedinfo_check(device, value, info, DEVICE_TYPE_SOCKET);
    if (err < 0)
...
static int intel_rapl_energy_limit_2_clamp_getter(
    const LikwidDevice_t device, char **value, uint32_t reg, const RaplDomainInfo *info)
{
    int err = getset_unusedinfo_check(device, value, info, DEVICE_TYPE_SOCKET);
    if (err < 0)
...
static int intel_rapl_energy_limit_2_clamp_setter(
    const LikwidDevice_t device, const char *value, uint32_t reg, const RaplDomainInfo *info)
{
    int err = getset_unusedinfo_check(device, value, info, DEVICE_TYPE_SOCKET);
    if (err < 0)
...
{
    if (intel_rapl_pkg_info.powerUnit == 0 && intel_rapl_pkg_info.energyUnit == 0 &&
        intel_rapl_pkg_info.timeUnit == 0) {
...
static int intel_rapl_pkg_limit_test_lock(void)
{
    return likwid_sysft_foreach_socket_testmsr_cb(
        MSR_PKG_RAPL_POWER_LIMIT, pkg_limit_test_lock_testFunc, NULL);
}
...
{
    return intel_rapl_energy_status_getter(
        device, value, MSR_PKG_ENERGY_STATUS, &intel_rapl_pkg_info);
}
...
static _SysFeature intel_rapl_pkg_features[] = {
    { "pkg_energy",
     ...
     intel_pkg_energy_status_getter,         NULL,
     DEVICE_TYPE_SOCKET, intel_pkg_energy_status_test,
     "J"    },
    { "pkg_tdp",
     "rapl", "Thermal Spec Power",
     intel_pkg_info_tdp,                     NULL,
     DEVICE_TYPE_SOCKET, intel_pkg_info_test,
     "W"    },
    { "pkg_min_limit",
     "rapl", "Minimum Power",
     intel_pkg_info_min_power,               NULL,
     DEVICE_TYPE_SOCKET, intel_pkg_info_test,
     "W"    },
    { "pkg_max_limit",
     "rapl", "Maximum Power",
     intel_pkg_info_max_power,               NULL,
     DEVICE_TYPE_SOCKET, intel_pkg_info_test,
     "W"    },
    { "pkg_max_time",
     "rapl", "Maximum Time",
     intel_pkg_info_max_time,                NULL,
     DEVICE_TYPE_SOCKET, intel_pkg_info_test,
     "s"    },
    { "pkg_limit_1",
     "rapl", "Long-term energy limit (PKG domain)",
     intel_pkg_energy_limit_1_getter,        intel_pkg_energy_limit_1_setter,
     DEVICE_TYPE_SOCKET, intel_pkg_energy_limit_test,
     "W"    },
    { "pkg_limit_1_time",
     "rapl", "Long-term time window (PKG domain)",
     intel_pkg_energy_limit_1_time_getter,   intel_pkg_energy_limit_1_time_setter,
     DEVICE_TYPE_SOCKET, intel_pkg_energy_limit_test,
     "s"    },
    { "pkg_limit_1_enable",
     "rapl", "Status of long-term energy limit (PKG domain)",
     intel_pkg_energy_limit_1_enable_getter, intel_pkg_energy_limit_1_enable_setter,
     DEVICE_TYPE_SOCKET, intel_pkg_energy_limit_test,
     "bool" },
    { "pkg_limit_1_clamp",
     "rapl", "Clamping status of long-term energy limit (PKG domain)",
     intel_pkg_energy_limit_1_clamp_getter,  intel_pkg_energy_limit_1_clamp_setter,
     DEVICE_TYPE_SOCKET, intel_pkg_energy_limit_test,
     "bool" },
    { "pkg_limit_2",
     "rapl", "Short-term energy limit (PKG domain)",
     intel_pkg_energy_limit_2_getter,        intel_pkg_energy_limit_2_setter,
     DEVICE_TYPE_SOCKET, intel_pkg_energy_limit_test,
     "W"    },
    { "pkg_limit_2_time",
     "rapl", "Short-term time window (PKG domain)",
     intel_pkg_energy_limit_2_time_getter,   intel_pkg_energy_limit_2_time_setter,
     DEVICE_TYPE_SOCKET, intel_pkg_energy_limit_test,
     "s"    },
    { "pkg_limit_2_enable",
     "rapl", "Status of short-term energy limit (PKG domain)",
     intel_pkg_energy_limit_2_enable_getter, intel_pkg_energy_limit_2_enable_setter,
     DEVICE_TYPE_SOCKET, intel_pkg_energy_limit_test,
     "bool" },
    { "pkg_limit_2_clamp",
     "rapl", "Clamping status of short-term energy limit (PKG domain)",
     intel_pkg_energy_limit_2_clamp_getter,  intel_pkg_energy_limit_2_clamp_setter,
     DEVICE_TYPE_SOCKET, intel_pkg_energy_limit_test,
     "bool" },
};
...
{
    if (intel_rapl_dram_info.powerUnit == 0 && intel_rapl_dram_info.energyUnit == 0 &&
        intel_rapl_dram_info.timeUnit == 0) {
        ...
        if ((info->model == HASWELL_EP) || (info->model == HASWELL_M1) ||
            (info->model == HASWELL_M2) || (info->model == BROADWELL_D) ||
            (info->model == BROADWELL_E) || (info->model == SKYLAKEX) ||
            (info->model == ICELAKEX1) || (info->model == ICELAKEX2) ||
            (info->model == XEON_PHI_KNL) || (info->model == XEON_PHI_KML)) {
...
{
    return intel_rapl_energy_status_getter(
        device, value, MSR_DRAM_ENERGY_STATUS, &intel_rapl_dram_info);
}
...
{
    return likwid_sysft_foreach_socket_testmsr_cb(
        MSR_DRAM_RAPL_POWER_LIMIT, dram_limit_test_lock_testFunc, NULL);
}

static int intel_dram_energy_limit_1_getter(const LikwidDevice_t device, char **value)
{
    return intel_rapl_energy_limit_1_getter(
        device, value, MSR_DRAM_RAPL_POWER_LIMIT, &intel_rapl_dram_info);
}
...
static _SysFeature intel_rapl_dram_features[] = {
    { "dram_energy",
     "rapl", "Current energy consumtion (DRAM domain)",
     intel_dram_energy_status_getter,         NULL,
     DEVICE_TYPE_SOCKET, intel_dram_energy_status_test,
     "J"    },
    { "dram_tdp",
     "rapl", "Thermal Spec Power",
     intel_dram_info_tdp,                     NULL,
     DEVICE_TYPE_SOCKET, intel_dram_info_test,
     "W"    },
    { "dram_min_limit",
     "rapl", "Minimum Power",
     intel_dram_info_min_power,               NULL,
     DEVICE_TYPE_SOCKET, intel_dram_info_test,
     "W"    },
    { "dram_max_limit",
     "rapl", "Maximum Power",
     intel_dram_info_max_power,               NULL,
     DEVICE_TYPE_SOCKET, intel_dram_info_test,
     "W"    },
    { "dram_max_time",
     "rapl", "Maximum Time",
     intel_dram_info_max_time,                NULL,
     DEVICE_TYPE_SOCKET, intel_dram_info_test,
     "s"    },
    { "dram_limit",
     "rapl", "Long-term energy limit (DRAM domain)",
     intel_dram_energy_limit_1_getter,        intel_dram_energy_limit_1_setter,
     DEVICE_TYPE_SOCKET, intel_dram_energy_limit_test,
     "W"    },
    { "dram_limit_time",
     "rapl", "Long-term time window (DRAM domain)",
     intel_dram_energy_limit_1_time_getter,   intel_dram_energy_limit_1_time_setter,
     DEVICE_TYPE_SOCKET, intel_dram_energy_limit_test,
     "s"    },
    { "dram_limit_enable",
     "rapl", "Status of long-term energy limit (DRAM domain)",
     intel_dram_energy_limit_1_enable_getter, intel_dram_energy_limit_1_enable_setter,
     DEVICE_TYPE_SOCKET, intel_dram_energy_limit_test,
     "bool" },
    { "dram_limit_clamp",
     "rapl", "Clamping status of long-term energy limit (DRAM domain)",
     intel_dram_energy_limit_1_clamp_getter,  intel_dram_energy_limit_1_clamp_setter,
     DEVICE_TYPE_SOCKET, intel_dram_energy_limit_test,
     "bool" },
};
...
{
    if (intel_rapl_psys_info.powerUnit == 0 && intel_rapl_psys_info.energyUnit == 0 &&
        intel_rapl_psys_info.timeUnit == 0) {
        CpuInfo_t info                  = get_cpuInfo();
...
{
    return intel_rapl_energy_status_getter(
        device, value, MSR_PLATFORM_ENERGY_STATUS, &intel_rapl_psys_info);
}
...
{
    return likwid_sysft_foreach_socket_testmsr_cb(
        MSR_PLATFORM_POWER_LIMIT, psys_limit_test_lock_testFunc, NULL);
}

static int intel_psys_energy_limit_1_getter(const LikwidDevice_t device, char **value)
{
    return intel_rapl_energy_limit_1_getter(
        device, value, MSR_PLATFORM_POWER_LIMIT, &intel_rapl_psys_info);
}
...
static _SysFeature intel_rapl_psys_features[] = {
    { "psys_energy",
     "rapl", "Current energy consumtion (PSYS domain)",
     intel_psys_energy_status_getter,         NULL,
     DEVICE_TYPE_SOCKET, intel_psys_energy_status_test,
     "J"    },
    { "psys_limit_1",
     "rapl", "Long-term energy limit (PSYS domain)",
     intel_psys_energy_limit_1_getter,        intel_psys_energy_limit_1_setter,
     DEVICE_TYPE_SOCKET, intel_psys_energy_limit_test,
     "W"    },
    { "psys_limit_1_time",
     "rapl", "Long-term time window (PSYS domain)",
     intel_psys_energy_limit_1_time_getter,   intel_psys_energy_limit_1_time_setter,
     DEVICE_TYPE_SOCKET, intel_psys_energy_limit_test,
     "s"    },
    { "psys_limit_1_enable",
     "rapl", "Status of long-term energy limit (PSYS domain)",
     intel_psys_energy_limit_1_enable_getter, intel_psys_energy_limit_1_enable_setter,
     DEVICE_TYPE_SOCKET, intel_psys_energy_limit_test,
     "bool" },
    { "psys_limit_1_clamp",
     "rapl", "Clamping status of long-term energy limit (PSYS domain)",
     intel_psys_energy_limit_1_clamp_getter,  intel_psys_energy_limit_1_clamp_setter,
     DEVICE_TYPE_SOCKET, intel_psys_energy_limit_test,
     "bool" },
    { "psys_limit_2",
     "rapl", "Short-term energy limit (PSYS domain)",
     intel_psys_energy_limit_2_getter,        intel_psys_energy_limit_2_setter,
     DEVICE_TYPE_SOCKET, intel_psys_energy_limit_test,
     "W"    },
    { "psys_limit_2_time",
     "rapl", "Short-term time window (PSYS domain)",
     intel_psys_energy_limit_2_time_getter,   intel_psys_energy_limit_2_time_setter,
     DEVICE_TYPE_SOCKET, intel_psys_energy_limit_test,
     "s"    },
    { "psys_limit_2_enable",
     "rapl", "Status of short-term energy limit (PSYS domain)",
     intel_psys_energy_limit_2_enable_getter, intel_psys_energy_limit_2_enable_setter,
     DEVICE_TYPE_SOCKET, intel_psys_energy_limit_test,
     "bool" },
    { "psys_limit_2_clamp",
     "rapl", "Clamping status of short-term energy limit (PSYS domain)",
     intel_psys_energy_limit_2_clamp_getter,  intel_psys_energy_limit_2_clamp_setter,
     DEVICE_TYPE_SOCKET, intel_psys_energy_limit_test,
     "bool" },
};
...
{
    if (intel_rapl_pp0_info.powerUnit == 0 && intel_rapl_pp0_info.energyUnit == 0 &&
        intel_rapl_pp0_info.timeUnit == 0) {
...
{
    return intel_rapl_energy_status_getter(
        device, value, MSR_PP0_ENERGY_STATUS, &intel_rapl_pp0_info);
}
...
{
    return likwid_sysft_foreach_socket_testmsr_cb(
        MSR_PP0_RAPL_POWER_LIMIT, pp0_limit_test_lock_testFunc, NULL);
}

static int intel_pp0_energy_limit_1_getter(const LikwidDevice_t device, char **value)
{
    return intel_rapl_energy_limit_1_getter(
        device, value, MSR_PP0_RAPL_POWER_LIMIT, &intel_rapl_pp0_info);
}
...
static _SysFeature intel_rapl_pp0_features[] = {
    { "pp0_energy",
     "rapl", "Current energy consumtion (PP0 domain)",
     intel_pp0_energy_status_getter, NULL,
     DEVICE_TYPE_SOCKET, intel_pp0_energy_status_test,
     "J" },
    { "pp0_limit",
     "rapl", "Long-term energy limit (PP0 domain)",
     intel_pp0_energy_limit_1_getter, intel_pp0_energy_limit_1_setter,
     DEVICE_TYPE_SOCKET, intel_pp0_energy_limit_test,
     "W" },
    { "pp0_limit_time",
     "rapl", "Long-term time window (PP0 domain)",
     intel_pp0_energy_limit_1_time_getter, intel_pp0_energy_limit_1_time_setter,
     DEVICE_TYPE_SOCKET, intel_pp0_energy_limit_test,
     "s" },
    { "pp0_limit_enable",
     "rapl", "Status of long-term energy limit (PP0 domain)",
     intel_pp0_energy_limit_1_enable_getter, intel_pp0_energy_limit_1_enable_setter,
     DEVICE_TYPE_SOCKET, intel_pp0_energy_limit_test,
     "bool" },
    { "pp0_limit_clamp",
     "rapl", "Clamping status of long-term energy limit (PP0 domain)",
     intel_pp0_energy_limit_1_clamp_getter, intel_pp0_energy_limit_1_clamp_setter,
     DEVICE_TYPE_SOCKET, intel_pp0_energy_limit_test,
     "bool" },
    { "pp0_policy",
     "rapl", "Balance Power Policy (PP0 domain)",
     intel_pp0_policy_getter, intel_pp0_policy_setter,
     DEVICE_TYPE_SOCKET, intel_pp0_policy_test },
};
...
{
    if (intel_rapl_pp1_info.powerUnit == 0 && intel_rapl_pp1_info.energyUnit == 0 &&
        intel_rapl_pp1_info.timeUnit == 0) {
...
{
    return intel_rapl_energy_status_getter(
        device, value, MSR_PP1_ENERGY_STATUS, &intel_rapl_pp1_info);
}
...
static int pp1_limit_test_lock(uint64_t msrData, void *value) { return field64(msrData, 31, 1); }

static int intel_rapl_pp1_limit_test_lock(void)
{
    return likwid_sysft_foreach_socket_testmsr_cb(
        MSR_PP1_RAPL_POWER_LIMIT, pp1_limit_test_lock, NULL);
}

static int intel_pp1_energy_limit_1_getter(const LikwidDevice_t device, char **value)
{
    return intel_rapl_energy_limit_1_getter(
        device, value, MSR_PP1_RAPL_POWER_LIMIT, &intel_rapl_pp1_info);
}
...
static _SysFeature intel_rapl_pp1_features[] = {
    { "pp1_energy",
     "rapl", "Current energy consumtion (PP1 domain)",
     intel_pp1_energy_status_getter,         NULL,
     DEVICE_TYPE_SOCKET, intel_pp1_energy_status_test,
     "uJ"         },
    { "pp1_limit",
     "rapl", "Long-term energy limit (PP1 domain)",
     intel_pp1_energy_limit_1_getter,        intel_pp1_energy_limit_1_setter,
     DEVICE_TYPE_SOCKET, intel_pp1_energy_limit_test,
     "mW"         },
    { "pp1_limit_time",
     "rapl", "Long-term time window (PP1 domain)",
     intel_pp1_energy_limit_1_time_getter,   intel_pp1_energy_limit_1_time_setter,
     DEVICE_TYPE_SOCKET, intel_pp1_energy_limit_test,
     "ms"         },
    { "pp1_limit_enable",
     "rapl", "Status of long-term energy limit (PP1 domain)",
     intel_pp1_energy_limit_1_enable_getter, intel_pp1_energy_limit_1_enable_setter,
     DEVICE_TYPE_SOCKET, intel_pp1_energy_limit_test,
     "bool"       },
    { "pp1_limit_clamp",
     "rapl", "Clamping status of long-term energy limit (PP1 domain)",
     intel_pp1_energy_limit_1_clamp_getter,  intel_pp1_energy_limit_1_clamp_setter,
     DEVICE_TYPE_SOCKET, intel_pp1_energy_limit_test,
     "bool"       },
    { "pp1_policy",
     "rapl", "Balance Power Policy (PP1 domain)",
     intel_pp1_policy_getter,                intel_pp1_policy_setter,
     DEVICE_TYPE_SOCKET, intel_pp1_policy_test,
     "bool[4..0]" },
};
