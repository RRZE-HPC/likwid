...
static cerr_t intel_cpu_l2_hwpf_register_test(bool *ok)
{
    return likwid_sysft_foreach_hwt_testmsr(ok, MSR_PREFETCH_ENABLE);
}

static cerr_t intel_cpu_l2_hwpf_getter(const LikwidDevice_t device, char** value)
{
    return likwid_sysft_readmsr_bit_to_string(device, MSR_PREFETCH_ENABLE, 0, true, value);
}

static cerr_t intel_cpu_l2_hwpf_setter(const LikwidDevice_t device, const char* value)
{
    return likwid_sysft_writemsr_bit_from_string(device, MSR_PREFETCH_ENABLE, 0, true, value);
}

static cerr_t intel_cpu_l2_adj_pf_getter(const LikwidDevice_t device, char** value)
{
    return likwid_sysft_readmsr_bit_to_string(device, MSR_PREFETCH_ENABLE, 1, true, value);
}

static cerr_t intel_cpu_l2_adj_pf_setter(const LikwidDevice_t device, const char* value)
{
    return likwid_sysft_writemsr_bit_from_string(device, MSR_PREFETCH_ENABLE, 1, true, value);
}

static cerr_t intel_cpu_l1_dcu_getter(const LikwidDevice_t device, char** value)
{
    return likwid_sysft_readmsr_bit_to_string(device, MSR_PREFETCH_ENABLE, 2, true, value);
}

static cerr_t intel_cpu_l1_dcu_setter(const LikwidDevice_t device, const char* value)
{
    return likwid_sysft_writemsr_bit_from_string(device, MSR_PREFETCH_ENABLE, 2, true, value);
}

static cerr_t intel_cpu_l1_dcu_ip_getter(const LikwidDevice_t device, char** value)
{
    return likwid_sysft_readmsr_bit_to_string(device, MSR_PREFETCH_ENABLE, 3, true, value);
}

static cerr_t intel_cpu_l1_dcu_ip_setter(const LikwidDevice_t device, const char* value)
{
...
static cerr_t intel_cpu_l2_multipath_pf_getter(const LikwidDevice_t device, char** value)
{
    return likwid_sysft_readmsr_bit_to_string(device, MSR_PREFETCH_ENABLE, 6, true, value);
}

static cerr_t intel_cpu_l2_multipath_pf_setter(const LikwidDevice_t device, const char* value)
{
...
/*********************************************************************************************************************/
static cerr_t intel_knl_l1_dcu_getter(const LikwidDevice_t device, char** value)
{
    return likwid_sysft_readmsr_bit_to_string(device, MSR_PREFETCH_ENABLE, 0, true, value);
}

static cerr_t intel_knl_l1_dcu_setter(const LikwidDevice_t device, const char* value)
{
    return likwid_sysft_writemsr_bit_from_string(device, MSR_PREFETCH_ENABLE, 0, true, value);
}

static cerr_t intel_knl_l2_hwpf_getter(const LikwidDevice_t device, char** value)
{
    return likwid_sysft_readmsr_bit_to_string(device, MSR_PREFETCH_ENABLE, 1, true, value);
}

static cerr_t intel_knl_l2_hwpf_setter(const LikwidDevice_t device, const char* value)
{
    return likwid_sysft_writemsr_bit_from_string(device, MSR_PREFETCH_ENABLE, 1, true, value);
}
...
static cerr_t intel_core2_l2_hwpf_register_test(bool *ok)
{
    return likwid_sysft_foreach_hwt_testmsr(ok, MSR_IA32_MISC_ENABLE);
}

static cerr_t intel_core2_l2_hwpf_getter(const LikwidDevice_t device, char** value)
{
    return likwid_sysft_readmsr_bit_to_string(device, MSR_IA32_MISC_ENABLE, 9, true, value);
}

static cerr_t intel_core2_l2_hwpf_setter(const LikwidDevice_t device, const char* value)
{
    return likwid_sysft_writemsr_bit_from_string(device, MSR_IA32_MISC_ENABLE, 9, true, value);
}

static cerr_t intel_core2_l2_adjpf_getter(const LikwidDevice_t device, char** value)
{
    return likwid_sysft_readmsr_bit_to_string(device, MSR_IA32_MISC_ENABLE, 19, true, value);
}

static cerr_t intel_core2_l2_adjpf_setter(const LikwidDevice_t device, const char* value)
{
    return likwid_sysft_writemsr_bit_from_string(device, MSR_IA32_MISC_ENABLE, 19, true, value);
}

static cerr_t intel_core2_l1_dcu_getter(const LikwidDevice_t device, char** value)
{
    return likwid_sysft_readmsr_bit_to_string(device, MSR_IA32_MISC_ENABLE, 37, true, value);
}

static cerr_t intel_core2_l1_dcu_setter(const LikwidDevice_t device, const char* value)
{
    return likwid_sysft_writemsr_bit_from_string(device, MSR_IA32_MISC_ENABLE, 37, true, value);
}

static cerr_t intel_core2_l1_dcu_ip_getter(const LikwidDevice_t device, char** value)
{
    return likwid_sysft_readmsr_bit_to_string(device, MSR_IA32_MISC_ENABLE, 39, true, value);
}

static cerr_t intel_core2_l1_dcu_ip_setter(const LikwidDevice_t device, const char* value)
{
...
static cerr_t intel_core2_ida_tester(bool *ok)
{
...
static cerr_t intel_core2_ida_getter(const LikwidDevice_t device, char** value)
{
    return likwid_sysft_readmsr_bit_to_string(device, MSR_IA32_MISC_ENABLE, 38, true, value);
}

static cerr_t intel_core2_ida_setter(const LikwidDevice_t device, const char* value) {
