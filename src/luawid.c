...
static int lua_likwid_init_rocm(lua_State *L)
{
    ...
    luaL_argcheck(L,
        numGpus >= 0,
        1,
        "GPU count must be greater than 0 (or zero to select all available GPUs)");
    ...
    lua_newtable(L);
    {
        ...
        for (size_t i = 0; i < (size_t)numGpus; i++) {
            ...
            lua_newtable(L);
            {
                ...
                if (err < 0)
                    return luaL_error(
                        L, "rocmon_getEventsOfGpu(%d) failed: %s", gpuId, strerror(-err));
...
static int lua_likwid_markerGetGroupInfo_rocm(lua_State *L)
{
    ...
    int err = rocmon_markerGetGroupInfo(groupId,
        &eventNames,
        &counterNames,
        &numEvents,
        &metricNames,
...
static int lua_likwid_markerGetRegionCounters_rocm(lua_State *L)
{
    ...
    int err = rocmon_markerGetRegionCounters(
        regionTag, groupId, gpuId, &numCounters, &counters, &numMetrics, &metrics);
    if (err < 0)
...
int __attribute__((visibility("default"))) luaopen_liblikwid(lua_State *L)
{
    ...
    lua_register(L, "likwid_getShortInfoOfGroup_rocm", lua_likwid_getShortInfoOfGroup_rocm);
    lua_register(L, "likwid_getLongInfoOfGroup_rocm", lua_likwid_getLongInfoOfGroup_rocm);
    ...
    lua_register(L, "likwid_getFormulaOfMetric_rocm", lua_likwid_getFormulaOfMetric_rocm);
    lua_register(L, "likwid_getNameOfGroup_rocm", lua_likwid_getNameOfGroup_rocm);
    lua_register(
        L, "likwid_markerInitResultsFromFile_rocm", lua_likwid_markerInitResultsFromFile_rocm);
