...
static void error_print_scope_recurse(FILE *handle, cerr_t *scope, int depth) {
    ...
    int line = 0;
    int error_val = 0;
    error_val_to_str_t error_val_to_str = strerror_const;
    ...
    if (error_val != 0) {
        if (error_val_to_str)
            fprintf(handle, "errval: %s%s (%d)%s, ", tty_underline, error_val_to_str(error_val), error_val, tty_reset);
        else
