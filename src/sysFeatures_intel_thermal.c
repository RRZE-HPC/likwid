...
static int intel_thermal_temperature_getter(const LikwidDevice_t device, bool core, char **value)
{
    ...
    err = likwid_sysft_readmsr(
        device, core ? IA32_THERM_STATUS : IA32_PACKAGE_THERM_STATUS, &therm_status_raw);
    if (err < 0)
        return err;
...
static int intel_thermal_tester(void) { return cpuid_hasFeature(TM2); }

static _SysFeature intel_thermal_features[] = {
    { "core_temp",
     "thermal", "Current CPU temperature (core)",
     intel_thermal_temperature_core_getter,   NULL,
     DEVICE_TYPE_CORE,   NULL,
     "degrees C" },
    { "pkg_temp",
     "thermal", "Current CPU temperature (package)",
     intel_thermal_temperature_socket_getter, NULL,
