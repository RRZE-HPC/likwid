 ...
 * Most notably it doesn't parse escaped strings correctly.
 * Neither are reserved symbols (e.h. '"') written in their escaped form.
 * Accordingly, this will produce malformed XML. Though, for the purposes
 * we use this library for, this is irrelevant, since it is only used for
 * processing hwloc topologies and not used provided input. */
