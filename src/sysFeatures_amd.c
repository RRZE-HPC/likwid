...
static _SysFeature amd_k19_cpu_prefetch_features[] = {
    { "l1_stream",
     "prefetch", "Stream prefetcher that uses history of memory access patterns to fetch additional "
        "sequential lines into L1 "
        "cache",                                                              amd_cpu_l1_stream_getter,
    ...
    { "l1_stride",
     "prefetch", "Stride prefetcher that uses memory access history of individual instructions to fetch "
        "additional lines into "
        "L1 cache when each access is a constant distance from the previous", amd_cpu_l1_stride_getter,
    ...
    { "l1_region",
     "prefetch", "Prefetcher that uses memory access history to fetch additional lines into L1 cache when "
        ...
        "given instruction tends to be followed by a consistent pattern of other accesses within a "
        "localized region",                                                   amd_cpu_l1_region_getter,
     amd_cpu_l1_region_setter, DEVICE_TYPE_HWTHREAD },
    { "l2_stream",
     "prefetch", "Stream prefetcher that uses history of memory access patterns to fetch additional "
        "sequential lines into L2 "
        "cache",                                                              amd_cpu_l2_stream_getter,
    ...
    { "up_down",
     "prefetch", "Prefetcher that uses memory access history to determine whether to fetch the next or "
        "previous line into L2 "
        "cache for all memory accesses",                                      amd_cpu_up_down_getter,
