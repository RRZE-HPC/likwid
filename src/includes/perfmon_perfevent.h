    ...
    PerfmonEventSet *eventSet)
{
    ...
    for (int i = 0; i < eventSet->numberOfEvents; i++) {
        ...
        case PMC:
            ...
            if (cpuid_info.vendor == FUJITSU_ARM && cpuid_info.part == FUJITSU_A64FX) {
                if (event->eventId == 0x308 || event->eventId == 0x309 || (event->eventId >= 0x314 && event->eventId <= 0x316) || (event->eventId >= 0x318 && event->eventId <= 0x31E) || event->eventId == 0x330 || event->eventId == 0x350 || event->eventId == 0x370 || event->eventId == 0x396 || event->eventId == 0x3E0 || event->eventId == 0x3E8) {
