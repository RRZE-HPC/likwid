...
DECLARE_NVPWFUNC(
    NVPW_CUDA_MetricsContext_Create, (NVPW_CUDA_MetricsContext_Create_Params * params));
...
DECLARE_NVPWFUNC(
    NVPW_RawMetricsConfig_AddMetrics, (NVPW_RawMetricsConfig_AddMetrics_Params * params));
...
DECLARE_NVPWFUNC(
    NVPW_CounterDataBuilder_Destroy, (NVPW_CounterDataBuilder_Destroy_Params * params));
DECLARE_NVPWFUNC(
...
DECLARE_CUPTIFUNC(
    cuptiProfilerHostGetSubMetrics, (CUpti_Profiler_Host_GetSubMetrics_Params * params));
...
static int link_perfworks_libraries(void)
{
    ...
    if (cuda_runtime_version >= 11040) {
        ...
        NVPW_MetricsEvaluator_ConvertMetricNameToMetricEvalRequestPtr = DLSYM_AND_CHECK(
            dl_libhost, "NVPW_MetricsEvaluator_ConvertMetricNameToMetricEvalRequest");
        NVPW_MetricsEvaluator_GetMetricRawDependenciesPtr =
            DLSYM_AND_CHECK(dl_libhost, "NVPW_MetricsEvaluator_GetMetricRawDependencies");
        NVPW_MetricsEvaluator_EvaluateToGpuValuesPtr =
        ...
        NVPA_RawMetricsConfig_CreatePtr =
    ...
    if (cuda_runtime_version >= 12060) {
        cuptiProfilerHostInitializePtr = DLSYM_AND_CHECK(dl_cupti, "cuptiProfilerHostInitialize");
    ...
    LIKWID_CU_CALL(
        cuDeviceGetAttributePtr(&curDeviceId, CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MAJOR, dev),
        return -EFAULT);
    GPUDEBUG_PRINT(DEBUGLEV_DEVELOP, "Run cuDeviceGetAttribute for minor CC");
    LIKWID_CU_CALL(
        cuDeviceGetAttributePtr(&curDeviceId, CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MINOR, dev),
        return -EFAULT);
...
static int perfworks_check_nv_context(NvmonDevice_t device, CUcontext currentContext)
{
    ...
    if (!device->context) {
        ...
        if (context_of_dev < 0) // && !device->context)
        {
            ...
            LIKWID_CU_CALL(
                cuDevicePrimaryCtxRetainPtr(&device->context, device->cuDevice), return -EFAULT);
            GPUDEBUG_PRINT(DEBUGLEV_DEVELOP,
...
static void cuptiProfiler_finalize()
{
    if (cuptiProfiler_initialized) {
        ...
        LIKWID_CUPTI_API_CALL(cuptiProfilerDeInitializePtr(&profilerDeInitializeParams),
...
static int nvmon_perfworks_populateEvents_nvpw(NvmonDevice *dev)
{
    ...
    LIKWID_NVPW_API_CALL(
        NVPW_MetricsContext_GetMetricNames_BeginPtr(&getMetricNameBeginParams), err = -EPERM;
        goto deinit);
    ...
    LIKWID_NVPW_API_CALL(NVPW_MetricsContext_GetMetricNames_EndPtr(&getMetricNameEndParams), );
...
deinit:
    ...
    LIKWID_NVPW_API_CALL(NVPW_MetricsContext_DestroyPtr(&metricsContextDestroyParams), );
    return err;
}
    ...
    uint8_t *availImage, NVPA_RawMetricRequest **requests)
{
    ...
    LIKWID_NVPW_API_CALL(
        NVPW_CUDA_MetricsEvaluator_CalculateScratchBufferSizePtr(&calculateScratchBufferSizeParam),
        return -1);
    ...
    LIKWID_NVPW_API_CALL(
        NVPW_CUDA_MetricsEvaluator_InitializePtr(&metricEvaluatorInitializeParams), free(scratch);
        return -1);
    ...
    for (int i = 0; i < events->qty; i++) {
        ...
        LIKWID_NVPW_API_CALL(NVPW_MetricsEvaluator_ConvertMetricNameToMetricEvalRequestPtr(
                                 &convertMetricToEvalRequest),
            free(scratch);
            return -1);
        ...
        LIKWID_NVPW_API_CALL(
            NVPW_MetricsEvaluator_GetMetricRawDependenciesPtr(&getMetricRawDependenciesParms),
            free(scratch);
            return -1);
    ...
    LIKWID_NVPW_API_CALL(
        NVPW_MetricsEvaluator_DestroyPtr(&metricEvaluatorDestroyParams), free(scratch);
        free(rawDeps);
        free(reqs);
        return -1);
    ...
    NVPA_MetricsContext *context, struct bstrList *events, NVPA_RawMetricRequest **requests)
{
    ...
    for (int i = 0; i < events->qty; i++) {
        ...
        LIKWID_NVPW_API_CALL(
            NVPW_MetricsContext_GetMetricProperties_BeginPtr(&getMetricPropertiesBeginParams),
            return -1);
        for (const char **dep = getMetricPropertiesBeginParams.ppRawMetricDependencies; *dep; ++dep)
            raw_metrics++;
        LIKWID_NVPW_API_CALL(
            NVPW_MetricsContext_GetMetricProperties_EndPtr(&getMetricPropertiesEndParams),
            return -1);
    }
    ...
    const char *chip, struct bstrList *events, uint8_t **configImage, uint8_t *availImage)
{
    ...
    if (cuda_runtime_version < 11040 && NVPA_RawMetricsConfig_CreatePtr) {
        ...
        LIKWID_NVPW_API_CALL(
            NVPW_CUDA_MetricsContext_CreatePtr(&metricsContextCreateParams), return -1;);
        ...
        LIKWID_NVPW_API_CALL(
            NVPA_RawMetricsConfig_CreatePtr(&metricsConfigOptions, &pRawMetricsConfig), ierr = -1;
        ...
        LIKWID_NVPW_API_CALL(
            NVPW_CUDA_RawMetricsConfig_Create_V2Ptr(&rawMetricsConfigCreateParams), free(reqs);
            return -1);
    ...
    if (availImage) {
        ...
        LIKWID_NVPW_API_CALL(
            NVPW_RawMetricsConfig_SetCounterAvailabilityPtr(&setCounterAvailabilityParams),
            ierr = -1;
            goto nvmon_perfworks_createConfigImage_out);
    }
    ...
    LIKWID_NVPW_API_CALL(NVPW_RawMetricsConfig_BeginPassGroupPtr(&beginPassGroupParams), ierr = -1;
        goto nvmon_perfworks_createConfigImage_out;);
    ...
    LIKWID_NVPW_API_CALL(NVPW_RawMetricsConfig_AddMetricsPtr(&addMetricsParams), ierr = -1;
        goto nvmon_perfworks_createConfigImage_out;);
    ...
    LIKWID_NVPW_API_CALL(NVPW_RawMetricsConfig_EndPassGroupPtr(&endPassGroupParams), ierr = -1;
        goto nvmon_perfworks_createConfigImage_out);
    ...
    LIKWID_NVPW_API_CALL(NVPW_RawMetricsConfig_GetNumPassesPtr(&getNumPassesParams), ierr = -1;
        goto nvmon_perfworks_createConfigImage_out);
    ...
    LIKWID_NVPW_API_CALL(
        NVPW_RawMetricsConfig_GenerateConfigImagePtr(&generateConfigImageParams), ierr = -1;
        goto nvmon_perfworks_createConfigImage_out);
    ...
    LIKWID_NVPW_API_CALL(NVPW_RawMetricsConfig_GetConfigImagePtr(&getConfigImageParams), ierr = -1;
        goto nvmon_perfworks_createConfigImage_out);
    ...
    LIKWID_NVPW_API_CALL(
        NVPW_RawMetricsConfig_GetConfigImagePtr(&getConfigImageParams), free(cimage); ierr = -1;
        goto nvmon_perfworks_createConfigImage_out);
...
nvmon_perfworks_createConfigImage_out:
    ...
    LIKWID_NVPW_API_CALL(
        NVPW_RawMetricsConfig_DestroyPtr(&rawMetricsConfigDestroyParams), return -1;);
    ...
    if (metricsContextCreateParams.pMetricsContext) {
        ...
        LIKWID_NVPW_API_CALL(
            NVPW_MetricsContext_DestroyPtr(&metricsContextDestroyParams), return -1;);
    }
    ...
    char *chip, struct bstrList *events, uint8_t **cdpImage)
{
    ...
    if (cuda_runtime_version < 12060) {
        ...
        LIKWID_NVPW_API_CALL(
            NVPW_CUDA_MetricsContext_CreatePtr(&metricsContextCreateParams), err = -EPERM;
    ...
    LIKWID_NVPW_API_CALL(
        NVPW_CounterDataBuilder_CreatePtr(&counterDataBuilderCreateParams), err = -EPERM;
        goto nvmon_perfworks_createCounterDataPrefixImage_out);
    ...
    LIKWID_NVPW_API_CALL(NVPW_CounterDataBuilder_AddMetricsPtr(&addMetricsParams), err = -EPERM;
        goto nvmon_perfworks_createCounterDataPrefixImage_out);
    ...
    LIKWID_NVPW_API_CALL(
        NVPW_CounterDataBuilder_GetCounterDataPrefixPtr(&getCounterDataPrefixParams), err = -1;
        goto nvmon_perfworks_createCounterDataPrefixImage_out);
    ...
    LIKWID_NVPW_API_CALL(
        NVPW_CounterDataBuilder_GetCounterDataPrefixPtr(&getCounterDataPrefixParams), free(cdp);
        err = -1;
        goto nvmon_perfworks_createCounterDataPrefixImage_out);
...
nvmon_perfworks_createCounterDataPrefixImage_out:
    ...
    LIKWID_NVPW_API_CALL(
        NVPW_CounterDataBuilder_DestroyPtr(&counterDataBuilderDestroyParams), err = -1);
    ...
    if (mc) {
        ...
        LIKWID_NVPW_API_CALL(
            NVPW_MetricsContext_DestroyPtr(&metricsContextDestroyParams), err = -1);
    }
    /*    for (i = 0; i < num_reqs; i++)*/
    /*    {*/
    /*        free((void*)reqs[i].pMetricName);*/
...
static int nvmon_perfworks_addEventSet(NvmonDevice_t device, const char *eventString)
{
    ...
    if (cuda_version >= 11000 && cuda_runtime_version >= 11000) {
        ...
        LIKWID_CUPTI_API_CALL(
            cuptiProfilerGetCounterAvailabilityPtr(&getCounterAvailabilityParams), return -EFAULT);
...
static int nvmon_perfworks_setupCounterImageData(NvmonEventSet *eventSet)
{
    ...
    LIKWID_CUPTI_API_CALL(
        cuptiProfilerCounterDataImageCalculateSizePtr(&calculateSizeParams), return -EFAULT);
    ...
    LIKWID_CUPTI_API_CALL(
        cuptiProfilerCounterDataImageInitializePtr(&initializeParams), return -EFAULT);
    ...
    LIKWID_CUPTI_API_CALL(
        cuptiProfilerCounterDataImageCalculateScratchBufferSizePtr(&scratchBufferSizeParams),
        return -EFAULT);
    ...
    LIKWID_CUPTI_API_CALL(
        cuptiProfilerCounterDataImageInitializeScratchBufferPtr(&initScratchBufferParams),
        return -EFAULT);
...
static int nvmon_perfworks_getMetricValue12(char *chip, NvmonEventSet *eventSet, double **values)
{
    ...
    LIKWID_NVPW_API_CALL(
        NVPW_CUDA_MetricsEvaluator_CalculateScratchBufferSizePtr(&scratchBufParams), err = -EPERM;
        goto cleanup);
    ...
    LIKWID_NVPW_API_CALL(NVPW_CUDA_MetricsEvaluator_InitializePtr(&initParams), err = -EPERM;
        goto cleanup);
    ...
    for (int i = 0; i < eventSet->numberOfEvents; i++) {
        ...
        LIKWID_NVPW_API_CALL(
            NVPW_MetricsEvaluator_ConvertMetricNameToMetricEvalRequestPtr(&convertParams),
            err = -EPERM;
            goto cleanup);
    }
    ...
    LIKWID_NVPW_API_CALL(NVPW_MetricsEvaluator_EvaluateToGpuValuesPtr(&evalParams), err = -EPERM;
        goto cleanup);
...
static int nvmon_perfworks_getMetricValue11(char *chip, NvmonEventSet *eventSet, double **values)
{
    ...
    LIKWID_NVPW_API_CALL(NVPW_CUDA_MetricsContext_CreatePtr(&metricsContextCreateParams), err = -1;
    ...
    LIKWID_NVPW_API_CALL(NVPW_CounterData_GetNumRangesPtr(&getNumRangesParams), err = -1;
        goto nvmon_perfworks_getMetricValue_out;);
    ...
    LIKWID_NVPW_API_CALL(NVPW_MetricsContext_SetCounterDataPtr(&setCounterDataParams), err = -1;
        goto nvmon_perfworks_getMetricValue_out;);
    ...
    LIKWID_NVPW_API_CALL(NVPW_MetricsContext_EvaluateToGpuValuesPtr(&evalToGpuParams), err = -1;
        free(gpuValues);
        goto nvmon_perfworks_getMetricValue_out;);
...
nvmon_perfworks_getMetricValue_out:
    ...
    LIKWID_NVPW_API_CALL(NVPW_MetricsContext_DestroyPtr(&metricsContextDestroyParams), err = -1);
    return err;
}
