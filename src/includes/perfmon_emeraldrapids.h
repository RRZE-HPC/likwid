 ...
 * =======================================================================================
 *
 *      Filename:  perfmon_emeraldrapids.h
 *
 *      Description:  Header File of perfmon module for Intel Emerald Rapids.
 *
 *      Version:   <VERSION>
 *      Released:  <DATE>
 *
...
uint64_t emr_metrics_start(int thread_id, RegisterIndex index, PerfmonEvent* event, PerfmonCounter* data)
{
    return (1ULL << 48);
}
...
int emr_start_uncore(int thread_id, RegisterIndex index, PerfmonEvent* event, PerfmonCounter* data)
{
    int cpu_id = groupSet->threads[thread_id].processorId;
    uint64_t counter1 = counter_map[index].counterRegister;
    PciDeviceIndex dev = counter_map[index].device;
...
int emr_stop_uncore(int thread_id, RegisterIndex index, PerfmonEvent* event, PerfmonCounter* data)
{
...
int emr_read_uncore(int thread_id, RegisterIndex index, PerfmonEvent* event, PerfmonCounter* data)
{
    uint64_t counter_result = 0x0;
    int cpu_id = groupSet->threads[thread_id].processorId;
    uint64_t counter1 = counter_map[index].counterRegister;
...
int emr_setup_uncore_fixed(int thread_id, RegisterIndex index, PerfmonEvent *event)
{
    int j;
    uint64_t flags = 0x0ULL;
    int cpu_id = groupSet->threads[thread_id].processorId;
...
int emr_setup_uncore_freerun(int thread_id, RegisterIndex index, PerfmonEvent *event)
{
    uint64_t flags = 0x0ULL;
    int cpu_id = groupSet->threads[thread_id].processorId;
    PciDeviceIndex dev = counter_map[index].device;
...
int perfmon_setupCounterThread_emeraldrapids(
        int thread_id,
        PerfmonEventSet* eventSet)
{
    ...
    for (int i=0;i < eventSet->numberOfEvents;i++)
    {
        ...
        {
            case FIXED:
                fixed_flags |= emr_fixed_setup(thread_id, index, event, data);
                break;
            case PMC:
                emr_pmc_setup(thread_id, index, event, data);
                break;
            case POWER:
            case THERMAL:
            case VOLTAGE:
            ...
            case HBM31FIX:
                if (haveLock)
                {
                    emr_setup_uncore_fixed(thread_id, index, event);
                }
                break;
/*            case MDEV0:*/
/*                emr_setup_uncore_freerun(thread_id, index, event);*/
/*                break;*/
            ...
            case PBOX15:
                if (haveLock) {
                    err = emr_setup_uncore(thread_id, index, event);
                    if (err < 0)
                    {
...
int perfmon_startCountersThread_emeraldrapids(int thread_id, PerfmonEventSet* eventSet)
{
    int haveLock = 0;
    uint64_t flags = 0x0ULL;
    ...
    for (int i=0;i < eventSet->numberOfEvents;i++)
    {
        if (eventSet->events[i].threadCounter[thread_id].init == TRUE)
        {
            ...
            {
                case FIXED:
                    flags |= emr_fixed_start(thread_id, index, event, data);
                    break;
                case PMC:
                    flags |= emr_pmc_start(thread_id, index, event, data);
                    break;
                case POWER:
                    emr_power_start(thread_id, index, event, data);
                    break;
                case METRICS:
                    flags |= emr_metrics_start(thread_id, index, event, data);
                    break;
                case THERMAL:
                case VOLTAGE:
                    break;
                ...
                case HBM31FIX:
                    if (haveLock)
                    {
                        emr_start_uncore_fixed(thread_id, index, event, data);
                    }
                    break;
/*                case MDEV0:*/
/*                    emr_start_uncore_freerun(thread_id, index, event, data);*/
/*                    break;*/
                case MBOX0:
                case MBOX1:
                case MBOX2:
                ...
                case PBOX15:
                    if (haveLock)
                    {
                        emr_start_uncore(thread_id, index, event, data);
                    }
                    break;
                default:
                    break;
...
uint32_t emr_fixed_stop(int thread_id, RegisterIndex index, PerfmonEvent* event, PerfmonCounter* data)
{
    ...
    EMR_CHECK_CORE_OVERFLOW(index+32);
    VERBOSEPRINTREG(cpu_id, counter1, LLU_CAST counter_result, "STOP_FIXED");
    data[thread_id].counterData = counter_result;
    return 0;
}

uint32_t emr_pmc_stop(int thread_id, RegisterIndex index, PerfmonEvent* event, PerfmonCounter* data)
{
    uint64_t counter_result = 0x0ULL;
    int cpu_id = groupSet->threads[thread_id].processorId;
    uint64_t counter1 = counter_map[index].counterRegister;
    CHECK_MSR_READ_ERROR(HPMread(cpu_id, MSR_DEV, counter1, &counter_result));
    EMR_CHECK_CORE_OVERFLOW(index-cpuid_info.perf_num_fixed_ctr);
...
int perfmon_stopCountersThread_emeraldrapids(int thread_id, PerfmonEventSet* eventSet)
{
    int haveLock = 0;
    int coffset = 0;
    uint64_t counter_result = 0x0ULL;
    ...
    for (int i=0;i < eventSet->numberOfEvents;i++)
    {
        if (eventSet->events[i].threadCounter[thread_id].init == TRUE)
        {
            ...
            {
                case FIXED:
                    emr_fixed_stop(thread_id, index, event, data);
                    break;
                case PMC:
                    emr_pmc_stop(thread_id, index, event, data);
                    break;
                case POWER:
                    emr_power_stop(thread_id, index, event, data);
                    break;

                case THERMAL:
                    emr_thermal_stop(thread_id, index, event, data);
                    break;

                case VOLTAGE:
                    emr_voltage_stop(thread_id, index, event, data);
                    break;

                case METRICS:
                    emr_metrics_stop(thread_id, index, event, data);
                    break;
                case MBOX0FIX:
                case MBOX1FIX:
                case MBOX2FIX:
                ...
                case HBM31FIX:
                    if (haveLock)
                    {
                        emr_stop_uncore_fixed(thread_id, index, event, data);
                    }
                    break;
/*                case MDEV0:*/
/*                    emr_stop_uncore_freerun(thread_id, index, event, data);*/
/*                    break;*/
                case MBOX0:
                case MBOX1:
                case MBOX2:
                ...
                case PBOX15:
                    if (haveLock)
                    {
                        emr_stop_uncore(thread_id, index, event, data);
                    }
                    break;
            }
        }
...
uint32_t emr_pmc_read(int thread_id, RegisterIndex index, PerfmonEvent* event, PerfmonCounter* data)
{
    uint64_t counter_result = 0x0ULL;
    int cpu_id = groupSet->threads[thread_id].processorId;
    uint64_t counter1 = counter_map[index].counterRegister;
...
int perfmon_readCountersThread_emeraldrapids(int thread_id, PerfmonEventSet* eventSet)
{
    int haveLock = 0;
    int coffset = 0;
    uint64_t flags = 0x0ULL;
    ...
    for (int i=0;i < eventSet->numberOfEvents;i++)
    {
        if (eventSet->events[i].threadCounter[thread_id].init == TRUE)
        {
            ...
            {
                case FIXED:
                    emr_fixed_read(thread_id, index, event, data);
                    break;
                case PMC:
                    emr_pmc_read(thread_id, index, event, data);
                    break;
                case POWER:
                    emr_power_read(thread_id, index, event, data);
                    break;

                case THERMAL:
                    emr_thermal_read(thread_id, index, event, data);
                    break;

                case VOLTAGE:
                    emr_voltage_read(thread_id, index, event, data);
                    break;

                case METRICS:
                    emr_metrics_read(thread_id, index, event, data);
                    break;
                ...
                case HBM31FIX:
                    if (haveLock)
                    {
                        emr_read_uncore_fixed(thread_id, index, event, data);
                    }
                    break;
                ...
                case PBOX15:
                    if (haveLock)
                    {
                        emr_read_uncore(thread_id, index, event, data);
                    }
                    break;
            }
        }
...
int perfmon_finalizeCountersThread_emeraldrapids(int thread_id, PerfmonEventSet* eventSet)
{
    int haveLock = 0;
    int haveTileLock = 0;
    int clearPBS = 0;
