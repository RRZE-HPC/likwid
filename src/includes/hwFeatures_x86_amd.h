...
int hwFeatures_init_x86_amd(int* num_features, _HWFeature **features)
{
    ...
    c = 0;
    while (amd_arch_features[c].family >= 0 && amd_arch_features[c].model >= 0)
    {
        if (amd_arch_features[c].family == cpuinfo->family && amd_arch_features[c].model == cpuinfo->model)
        {
            feature_list = amd_arch_features[c].features;
            break;
        }
        c++;
    }
