...
const char *strerror_const(int error_val);
...
#define ERROR_SET_ERRNO(fmt, ...) error_set(__FILE__, __func__, __LINE__, errno, strerror_const, (fmt), __VA_ARGS__)
#define ERROR_APPEND(fmt, ...) error_append(__FILE__, __func__, __LINE__, 0, NULL, (fmt), __VA_ARGS__)
#define ERROR_APPEND_ERRNO(fmt, ...) error_append(__FILE__, __func__, __LINE__, errno, strerror_const, (fmt), __VA_ARGS__)
