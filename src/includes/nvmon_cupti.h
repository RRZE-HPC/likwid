...
DECLARECUPTIFUNC(
    cuptiEventGroupGetAttribute, (CUpti_EventGroup eventGroup, CUpti_EventGroupAttribute attrib,
                                     size_t *valueSize, void *value));
DECLARECUPTIFUNC(cuptiDeviceGetEventDomainAttribute,
    (CUdevice device, CUpti_EventDomainID eventDomain, CUpti_EventDomainAttribute attrib,
        size_t *valueSize, void *value));
DECLARECUPTIFUNC(cuptiEventGroupReadEvent,
    (CUpti_EventGroup eventGroup, CUpti_ReadEventFlags flags, CUpti_EventID event,
        size_t *eventValueBufferSizeBytes, uint64_t *eventValueBuffer));
DECLARECUPTIFUNC(cuptiEventGroupSetAttribute,
    (CUpti_EventGroup eventGroup, CUpti_EventGroupAttribute attrib, size_t valueSize, void *value));
...
DECLARECUPTIFUNC(cuptiEventGroupSetsCreate,
    (CUcontext context, size_t eventIdArraySizeBytes, CUpti_EventID *eventIdArray,
...
int nvmon_cupti_createDevice(int id, NvmonDevice *dev)
{
    ...
    if (!eventDomainIds)
        return -ENOMEM;
    CUPTI_CALL((*cuptiDeviceEnumEventDomainsPtr)(dev->cuDevice, &domainarraysize, eventDomainIds),
        return -1);
    ...
    for (unsigned j = 0; j < numDomains; j++) {
        ...
        for (unsigned k = 0; k < domainNumEvents; k++) {
            ...
            CUPTI_CALL((*cuptiEventGetAttributePtr)(
                           eventId, CUPTI_EVENT_ATTR_NAME, &tmpSizeBytesName, event->name),
                return -1);
            CUPTI_CALL((*cuptiEventGetAttributePtr)(eventId,
...
int nvmon_cupti_addEventSets(NvmonDevice_t device, const char *eventString)
{
    ...
    CUPTI_CALL(
        (*cuptiSetEventCollectionModePtr)(device->context, CUPTI_EVENT_COLLECTION_MODE_KERNEL),
        return -EFAULT);
    ...
    for (i = 0; i < eventtokens->qty; i++) {
        ...
        if (!event) {
            ...
            CUPTI_CALL((*cuptiEventGroupSetsCreatePtr)(
                           device->context, s, devEventSet->cuEventIDs, &cuEventSets),
                devEventSet->numberOfEvents--;);
...
int nvmon_cupti_setupCounters(NvmonDevice_t device, NvmonEventSet *eventSet)
{
    ...
    CUPTI_CALL((*cuptiEventGroupSetsCreatePtr)(
                   device->context, grpEventIdsSize, eventSet->cuEventIDs, &cuEventSets),
        return -1;);
    ...
    for (unsigned j = 0; j < cuEventSets->numSets; j++) {
        ...
        for (unsigned k = 0; k < groupset->numEventGroups; k++) {
            ...
            CUPTI_CALL((*cuptiEventGroupGetAttributePtr)(
                           group, CUPTI_EVENT_GROUP_ATTR_NUM_EVENTS, &sizeofuint32t, &numGEvents),
                return -EFAULT);
            // Get the CUPTI event IDs in the group
            CUPTI_CALL((*cuptiEventGroupGetAttributePtr)(
                           group, CUPTI_EVENT_GROUP_ATTR_EVENTS, &grpEventIdsSize, grpEventIds),
                return -EFAULT);
            // If we don't set this, each event has only a single instance but we want to measure all instances
            CUPTI_CALL((*cuptiEventGroupSetAttributePtr)(group,
            ...
            CUPTI_CALL(
                (*cuptiEventGroupGetAttributePtr)(
                    group, CUPTI_EVENT_GROUP_ATTR_INSTANCE_COUNT, &sizeofuint32t, &numGInstances),
                return -EFAULT);
            ...
            for (unsigned l = 0; l < numGEvents; l++) {
                for (int m = 0; m < eventSet->numberOfEvents; m++) {
                    if (eventSet->cuEventIDs[m] == grpEventIds[l]) {
                        ...
                        CUPTI_CALL((*cuptiDeviceGetEventDomainAttributePtr)(device->cuDevice,
                                       did,
    ...
    for (int i = 0; i < nvGroupSet->numberOfGPUs; i++) {
        ...
        for (int j = 0; j < device->numActiveEvents; j++) {
            ...
            GPUDEBUG_PRINT(DEBUGLEV_DEVELOP,
                ...
                event->cuEventId,
                device->deviceId);
            CUPTI_CALL((*cuptiEventGroupReadEventPtr)(event->cuGroup,
