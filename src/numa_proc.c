...
#define mbind(start, len, nmask, maxnode, flags)                                                   \
    syscall(SYS_mbind, (start), len, MPOL_BIND, (nmask), maxnode, flags)
#endif

/* #####   FUNCTION DEFINITIONS  -  LOCAL TO THIS SOURCE FILE   ########### */

static int proc_findProcessor(uint32_t nodeId, uint32_t coreId)
{
    int i = 0;
...
static int setConfiguredNodes(void)
{
    DIR *dir                = NULL;
...
static int get_numaNodes(int *array, int maxlen)
{
    DIR *dir          = NULL;
    struct dirent *de = NULL;
    int count         = 0;
...
static void nodeMeminfo(int node, uint64_t *totalMemory, uint64_t *freeMemory)
{
    FILE *fp;
    bstring filename;
                ...
                for (j = 0; j < unitSize; j++) {
                    if (val & (1UL << j)) {
                        if (count < cpuid_topology.numHWThreads) {
                            ...
                            ERROR_PRINT(
                                "Number Of threads %d too large for NUMA node %d", count, node);
                            return -EFAULT;
                        }
                        count++;
                    }
...
static int nodeDistanceList(int node, int numberOfNodes, uint32_t **list)
{
    FILE *fp;
    bstring filename;
    int count = 0;
...
int proc_numa_init(void)
{
    ...
    for (i = 0; i < numa_info.numberOfNodes; i++) {
        ...
        numa_info.nodes[i].numberOfProcessors =
            nodeProcessorList(id, &numa_info.nodes[i].processors);
        /*        int check = 0;*/
        ...
        numa_info.nodes[i].numberOfDistances =
            nodeDistanceList(id, numa_info.numberOfNodes, &numa_info.nodes[i].distances);
...
void proc_numa_setInterleaved(const int *processorList, int numberOfProcessors)
{
    long i;
    int j;
    int ret                     = 0;
...
void proc_numa_setMembind(const int *processorList, int numberOfProcessors)
{
    long i;
    int j;
    int ret                     = 0;
...
void proc_numa_membind(void *ptr, size_t size, int domainId)
{
    int ret            = 0;
    unsigned long mask = 0UL;
