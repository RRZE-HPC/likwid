        ...
        for (int i = 0; i < nvmon_getNumberOfEvents(gid); i++) {
            for (int j = 0; j < nvmon_numgpus; j++) {
                fprintf(output_file,
                    "Nvmon, %d, %f, %s, %f, %f\n",
                    nvmon_gpulist[j],
                    nvmon_getTimeOfGroup(nvmon_gpulist[j]),
                    nvmon_getEventName(gid, i),
                    nvmon_getResult(gid, i, j),
                    nvmon_getLastResult(gid, i, j));
            }
        }
    }
    fflush(output_file);
        ...
        for (int i = 0; i < nvmon_getNumberOfEvents(gid); i++) {
            for (int j = 0; j < nvmon_numgpus; j++) {
                fprintf(output_file,
                    "Nvmon, %d, %f, %s, %f, %f\n",
                    nvmon_gpulist[j],
                    nvmon_getTimeToLastReadOfGroup(nvmon_gpulist[j]),
                    nvmon_getEventName(gid, i),
                    nvmon_getResult(gid, i, j),
                    nvmon_getLastResult(gid, i, j));
            }
        ...
        for (int i = 0; i < rocmon_getNumberOfEvents(gid); i++) {
            for (int j = 0; j < rocmon_numgpus; j++) {
                fprintf(output_file,
                    "Rocmon, %d, %f, %s, %f, %f\n",
                    rocmon_gpulist[j],
                    rocmon_getTimeToLastReadOfGroup(rocmon_gpulist[j]),
                    rocmon_getEventName(gid, i),
                    rocmon_getResult(j, gid, i),
                    rocmon_getLastResult(j, gid, i));
            }
...
Main
*/
__attribute__((visibility("default"))) int __libc_start_main(int (*main)(int, char **, char **),
    int argc, char **ubp_av, void (*init)(void), void (*fini)(void), void (*rtld_fini)(void),
    void(*stack_end))
{
    int ret = 0;
