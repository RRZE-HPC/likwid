...
local function examples()
    ...
    print_stdout("  # List all performance groups")
    print_stdout("  $ likwid-perfctr -a")
    print_stdout()
    print_stdout("  # List all events and counters")
    print_stdout("  $ likwid-perfctr -e")
    print_stdout()
    print_stdout("  # List all events and suitable counters for events with 'L2' in them")
    print_stdout("  $ likwid-perfctr -E L2")
    print_stdout()
    print_stdout("  # Run command on CPU 2 and measure performance group CLOCK")
    print_stdout("  $ likwid-perfctr -C 2 -g CLOCK ./a.out")
    if likwid.nvSupported() then
        print_stdout()
        print_stdout("  # Run command and measure on GPU 1 the performance group FLOPS_DP (Only with NVMarkerAPI)")
        print_stdout("  $ likwid-perfctr -G 1 -W FLOPS_DP -m ./a.out")
        print_stdout()
        print_stdout("  # It is possible to combine CPU and GPU measurements (with MarkerAPI and NVMarkerAPI)")
        print_stdout("  $ likwid-perfctr -C 2 -g CLOCK -G 1 -W FLOPS_DP -m ./a.out")
    end
    if likwid.rocmSupported() then
        print_stdout()
        print_stdout("  # Run command and measure on GPU 1 the performance group PCI (Only with ROCmMarkerAPI)")
        print_stdout("  $ likwid-perfctr -I 1 -R PCI -m ./a.out")
        print_stdout()
        print_stdout("  # It is possible to combine CPU and GPU measurements (with MarkerAPI and ROCmMarkerAPI)")
        print_stdout("  $ likwid-perfctr -C 2 -g CLOCK -I 1 -R PCI -m ./a.out")
    end
end
...
local function usage(config)
    version()
    print_stdout()
    print_stdout("A tool to read out performance counter registers on x86, ARM and POWER processors.")
    print_stdout()
    print_stdout("General Options:")
    ...
    print_stdout("  -c <list>                Processor ids to measure (required), e.g. 1,2-4,8")
    print_stdout("  -C <list>                Processor ids to pin threads and measure, e.g. 1,2-4,8")
    print_stdout("                           For information about the <list> syntax, see likwid-pin")
    if likwid.nvSupported() then
        print_stdout("  -G, --gpus <list>        List of CUDA GPUs to monitor")
    end
    if likwid.rocmSupported() then
        print_stdout("  -I <list>                List of ROCm GPUs to monitor")
    end
    print_stdout("  -g, --group <string>     Performance group or custom event set string for CPU monitoring")
    if likwid.nvSupported() then
        print_stdout("  -W, --cudagroup <string> Performance group or custom event set string for GPU monitoring")
    end
    if likwid.rocmSupported() then
        print_stdout("  -R, --rocmgroup <string> Performance group or custom event set string for ROCm GPU monitoring")
    end
    print_stdout("  -H                       Get group help (together with -g switch)")
    ...
    print_stdout("  -a                       List available performance groups")
    print_stdout("  -e                       List available events and counter registers")
    print_stdout("  -E <string>              List available events and corresponding counters that match <string>")
    print_stdout("  -i, --info               Print CPU info")
    print_stdout("  -T <time>                Switch eventsets with given frequency")
    print_stdout("  -f, --force              Force overwrite of registers if they are in use")
    print_stdout()
    print_stdout("Mode Options:")
    print_stdout("  -S <time>                Stethoscope mode with duration in s, ms or us, e.g 20ms")
    print_stdout("  -t <time>                Timeline mode with frequency in s, ms or us, e.g. 300ms")
    print_stdout("                           The output format (to stderr) is:")
    print_stdout("                             <groupID> <nrEvents> <nrThreads> <Timestamp> <Event1_Thread1> <Event1_Thread2> ... <EventN_ThreadN>")
    print_stdout("                           or")
    print_stdout("                             <groupID> <nrEvents> <nrThreads> <Timestamp> <Metric1_Thread1> <Metric1_Thread2> ... <MetricN_ThreadN>")
    print_stdout("  -m, --marker             Use Marker API inside code")
    print_stdout()
    print_stdout("Output options:")
    print_stdout("  -o, --output <file>      Store output to file. (Optional: Apply text filter according to filename suffix)")
    print_stdout("  -O                       Output easily parsable CSV instead of fancy tables")
    print_stdout("  --stats                  Always print statistics table")
    if config and config["daemonMode"] == -1 then
        print_stdout()
        print_stdout("perf_event specific options:")
        print_stdout("  --perfpid <pid>          Measure given PID")
        print_stdout("  --execpid                Use the PID of wrapped application for measurements")
    end
    print_stdout()
    examples()
end
