...
local function usage()
    ...
    print_stdout()
    print_stdout("General Information:")
    ...
    for i, line in pairs(likwid.stringsplit(t, "\n")) do
        if (not line:match("^-")) and
           ...
           (not line:match("STAT")) then
            ...
            elseif parse_reg_info and line:match("TABLE,Region (.+),Group (%d+) Raw,([^%s]+),") then
                current_region, gidx, gname  = line:match("TABLE,Region (.+),Group (%d+) Raw,([^%s]+),")
                gidx = tonumber(gidx)
