...
local function examples()
    ...
    print_stdout("  # Pin to hardware threads 0,4,5 and 6 using physical numbering")
    print_stdout("  $ likwid-pin -c 0,4-6 ./app")
    print_stdout()
    print_stdout("  # Pin to hardware threads using logical thread numberings in physical cores first sorted list")
    print_stdout("  $ likwid-pin -c N:0,4-6 ./app")
    print_stdout()
    print_stdout("  # Expressions based thread list generation with compact processor numbering")
    print_stdout("  # This will generate a compact list of thread to processor mapping for the node domain with 8 threads.")
    print_stdout("  $ likwid-pin -c E:N:8 ./app")
    print_stdout()
    print_stdout("  # Scatter policy among thread domain type")
    ...
    print_stdout("  $ likwid-pin -c M:scatter ./app")
    print_stdout()
end
...
local function usage()
    version()
    print_stdout()
    print_stdout("A tool to pin a program including threads.")
    print_stdout()
    ...
    print_stdout("                        Following syntax variants are available for expression:")
    ...
    print_stdout("  1. Logical numbering inside node.")
    print_stdout("     e.g. -c N:0,1,2,3 for the first 4 physical cores of the node")
    print_stdout("  2. Logical numbering inside socket.")
    print_stdout("     e.g. -c S0:0-1 for the first 2 physical cores of the first socket")
    print_stdout("  3. Logical numbering inside last level cache group.")
    print_stdout("     e.g. -c C0:0-3  for the first 4 physical cores of the first last level cache domain")
    print_stdout("  4. Logical numbering inside NUMA domain.")
    print_stdout("     e.g. -c M0:0-3 for the first 4 physical cores of the first NUMA domain")
    print_stdout()
    print_stdout("  You can also mix domains separated by '@',")
    print_stdout("  e.g. -c S0:0-3@S1:0-3 for the 4 first physical cores of the first two sockets.")
    print_stdout()
