...
local function examples()
    ...
    print_stdout("  NOTE: Not all examples shown here may work on your hardware.")
    print_stdout()
    print_stdout("  # List all sysfeatures values for Socket 0 and 1.")
    print_stdout("  # Please keep in mind that this only prints values applicable to Socket Domain.")
    print_stdout("  $ likwid-sysfeatures --list --devices S:0,1")
    print_stdout()
    print_stdout("  # List available sysfeatures for Cores 0, 1, 4, 5, 6, 7")
    print_stdout("  $ likwid-sysfeatures --list --devices C:0,1,4-7")
    print_stdout()
    print_stdout("  # Get RAPL energy reading for Socket 0")
    print_stdout("  $ likwid-sysfeatures --get rapl.pkg_energy --devices S:0")
    print_stdout()
    print_stdout("  # Set power limit 1 on Socket 0 to 150W via AMD HSMP")
    print_stdout("  $ likwid-sysfeatures --set hsmp.pkg_power_limit_cur=150 --devices S:0")
end

local function usage()
    version()
    print_stdout()
    print_stdout("A tool to list and modify the states of system/OS features.")
    print_stdout()
