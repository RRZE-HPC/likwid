...
int power_init(int cpuId)
{
    ...
    case P6_FAMILY:
        ...
        break;
    case ZEN_FAMILY:
        if (cpuid_info.model == ZEN_RYZEN || cpuid_info.model == ZENPLUS_RYZEN ||
            cpuid_info.model == ZENPLUS_RYZEN2 || cpuid_info.model == ZEN2_RYZEN ||
            cpuid_info.model == ZEN2_RYZEN2 || cpuid_info.model == ZEN2_RYZEN3 ||
            cpuid_info.model == ZEN3_RYZEN || cpuid_info.model == ZEN3_RYZEN2 ||
            cpuid_info.model == ZEN3_RYZEN3 || cpuid_info.model == ZEN3_EPYC_TRENTO) {
            cpuid_info.turbo   = 0;
            power_info.hasRAPL = 1;
            numDomains         = 2;
            ...
            if (err) {
                ...
                if (!core_limits) {
                    ...
                    if (power_info.turbo.numSteps > 8) {
                        ...
                        if (err) {
                            ERROR_PRINT(
                                "Cannot read core limits from %s", "MSR_TURBO_RATIO_LIMIT1");
                            flag_vals[1] = 0;
                        }
                    }
                    if (power_info.turbo.numSteps > 16) {
                        ...
                        if (err) {
                            ERROR_PRINT(
                                "Cannot read core limits from %s", "MSR_TURBO_RATIO_LIMIT2");
                            flag_vals[2] = 0;
                        }
                    }
                    if (power_info.turbo.numSteps > 24) {
                        ...
                        if (err) {
                            ERROR_PRINT(
                                "Cannot read core limits from %s", "MSR_TURBO_RATIO_LIMIT3");
                            flag_vals[3] = 0;
                        }
                    }
                    ...
                    for (int i = 1; i < power_info.turbo.numSteps; i++) {
                        ...
                        power_info.turbo.steps[i] =
                            busSpeed * (double)field64(flag_vals[flag_idx], (i % 8) * 8, 8);
                    ...
                    if (err) {
                        ERROR_PRINT(
                            "Cannot read core limits from %s", "MSR_TURBO_RATIO_LIMIT_CORES");
                        flags_cores = 0;
                    }
                    for (int i = 0; i < 8; i++) {
                        ...
                        for (int j = num_cores_befores;
                            j < num_cores && insert < power_info.turbo.numSteps;
                            j++) {
    ...
    if (power_info.hasRAPL) {
        ...
        if (err == 0) {
            ...
            if (cpuid_info.family == P6_FAMILY &&
                ((cpuid_info.model == HASWELL_EP) || (cpuid_info.model == HASWELL_M1) ||
                    (cpuid_info.model == HASWELL_M2) || (cpuid_info.model == BROADWELL_D) ||
                    (cpuid_info.model == BROADWELL_E) || (cpuid_info.model == SKYLAKEX) ||
                    (cpuid_info.model == ICELAKEX1) || (cpuid_info.model == ICELAKEX2) ||
                    (cpuid_info.model == XEON_PHI_KNL) || (cpuid_info.model == XEON_PHI_KML))) {
            ...
            for (i = 0; i < numDomains; i++) {
                ...
                if (limit_regs[i] != 0x0) {
                    ...
                    if (err == 0) {
                        ...
                        DEBUG_PRINT(DEBUGLEV_DETAIL,
                            "Deactivating limit register for RAPL domain %s",
                            power_names[i]);
                        limit_regs[i] = 0x0;
                    }
                }
                if (info_regs[i] != 0x0) {
                    ...
                    if (err == 0) {
                        ...
                        power_info.domains[i].tdp =
                            (double)extractBitField(flags, 15, 0) * power_info.powerUnit;
                        if (cpuid_info.model != ATOM_SILVERMONT_C) {
                            power_info.domains[i].minPower =
                                (double)extractBitField(flags, 15, 16) * power_info.powerUnit;
                            power_info.domains[i].maxPower =
                                (double)extractBitField(flags, 15, 32) * power_info.powerUnit;
                            if (power_info.domains[i].minPower > power_info.domains[i].maxPower)
                                power_info.domains[i].minPower = 0;
                            power_info.domains[i].maxTimeWindow =
                                (double)extractBitField(flags, 7, 48) * power_info.timeUnit;
                        }
                    } else {
                        DEBUG_PRINT(DEBUGLEV_DETAIL,
                            "Deactivating info register for RAPL domain %s",
                            power_names[i]);
                        info_regs[i] = 0x0;
                    }
                }
                if (policy_regs[i] != 0x0) {
                    ...
                    if (err == 0) {
                        ...
                        DEBUG_PRINT(DEBUGLEV_DETAIL,
                            "Deactivating policy register for RAPL domain %s",
                            power_names[i]);
                        policy_regs[i] = 0x0;
                    }
                }
                if (perf_regs[i] != 0x0) {
                    ...
                    if (err == 0) {
                        ...
                        DEBUG_PRINT(DEBUGLEV_DETAIL,
                            "Deactivating perf register for RAPL domain %s",
                            power_names[i]);
                        perf_regs[i] = 0x0;
                    }
                }
            ...
            DEBUG_PRINT(DEBUGLEV_INFO,
                "Cannot gather values from unit register 0x%X. deactivating RAPL support",
                unit_reg);
            power_info.hasRAPL = 0;
        }
        ...
        if (cpuid_info.family == ZEN3_FAMILY &&
            (cpuid_info.model == ZEN4_RYZEN || cpuid_info.model == ZEN4_RYZEN2 ||
                cpuid_info.model == ZEN4_RYZEN3 || cpuid_info.model == ZEN4_EPYC ||
                cpuid_info.model == ZEN4_EPYC_BERGAMO)) {
...
int power_perfGet(int cpuId, PowerType domain, uint32_t *status)
{
    ...
    *status = 0x0U;
    ...
    if (power_info.domains[domain].supportFlags & POWER_DOMAIN_SUPPORT_PERF) {
        ...
        if (err) {
            ERROR_PRINT("Failed to get power perf value for domain %s on CPU %d",
                power_names[domain],
                cpuId);
            return -EFAULT;
        }
...
int power_limitSet(int cpuId, PowerType domain, double power, double time, int doClamping)
{
    ...
    if (power_info.domains[domain].supportFlags & POWER_DOMAIN_SUPPORT_LIMIT) {
        ...
        if (err) {
            ERROR_PRINT(
                "Failed to set power limit for domain %s on CPU %d", power_names[domain], cpuId);
            return -EFAULT;
        }
    }
    return 0;
...
int power_limitState(int cpuId, PowerType domain)
{
    ...
    if (power_info.domains[domain].supportFlags & POWER_DOMAIN_SUPPORT_LIMIT) {
        err = HPMread(cpuId, MSR_DEV, limit_regs[domain], &flags);
        if (err) {
            ERROR_PRINT("Failed to activate power limit for domain %s on CPU %d",
                power_names[domain],
                cpuId);
            return -EFAULT;
        }
...
int power_limitDectivate(int cpuId, PowerType domain)
{
    ...
    if (power_info.domains[domain].supportFlags & POWER_DOMAIN_SUPPORT_LIMIT) {
        err = HPMread(cpuId, MSR_DEV, limit_regs[domain], &flags);
        if (err) {
            ERROR_PRINT("Failed to deactivate power limit for domain %s on CPU %d",
                power_names[domain],
                cpuId);
            return -EFAULT;
        }
...
int power_policySet(int cpuId, PowerType domain, uint32_t priority)
{
    ...
    if (power_info.domains[domain].supportFlags & POWER_DOMAIN_SUPPORT_POLICY) {
        ...
        if (err) {
            ERROR_PRINT(
                "Failed to set power policy for domain %s on CPU %d", power_names[domain], cpuId);
            return -EFAULT;
        }
    }
    return 0;
...
int power_policyGet(int cpuId, PowerType domain, uint32_t *priority)
{
    ...
    *priority = 0x0U;
    ...
    if (power_info.domains[domain].supportFlags & POWER_DOMAIN_SUPPORT_POLICY) {
        ...
        if (err) {
            ERROR_PRINT(
                "Failed to get power policy for domain %s on CPU %d", power_names[domain], cpuId);
            return -EFAULT;
        }
    }
    return 0;
...
PowerInfo_t get_powerInfo(void) { return &power_info; }
