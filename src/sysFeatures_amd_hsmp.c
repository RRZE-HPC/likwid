...
static cerr_t hsmp_raw(uint32_t socket, uint32_t msg_id, const uint32_t *args, uint16_t argCount, uint32_t *result, uint16_t resultCount)
{
    ...
    if (fd < 0)
        return ERROR_SET_ERRNO("Failed to open /dev/hsmp");
    ...
    if (err < 0) {
        close(fd);
        return ERROR_SET_ERRNO("ioctl failed");
    }
...
static cerr_t hsmp_arg0_res1_as_u32(LikwidDevice_t dev, uint32_t msg_id, char **value)
{
    uint32_t result;
...
static cerr_t amd_hsmp_tester(bool *ok)
{
    ...
    uint32_t response;
    if (hsmp_raw(0, HSMP_TEST, &TEST_NUM, 1, &response, 1)) {
        *ok = false;
        return NULL;
    }
    *ok = true;
...
static cerr_t amd_hsmp_test_ver(bool *ok, uint32_t test_ver)
{
    *ok = false;
    if (test_ver == 0)
        return NULL;
    uint32_t proto_ver;
    if (hsmp_raw(0, HSMP_GET_PROTO_VER, NULL, 0, &proto_ver, 1))
        return NULL;
...
static cerr_t amd_hsmp_test_ver1(bool *ok) { return amd_hsmp_test_ver(ok, 1); }
static cerr_t amd_hsmp_test_ver2(bool *ok) { return amd_hsmp_test_ver(ok, 2); }
static cerr_t amd_hsmp_test_ver3(bool *ok) { return amd_hsmp_test_ver(ok, 3); }
//static cerr_t amd_hsmp_test_ver4(bool *ok) { return amd_hsmp_test_ver(ok, 4); } // there is only one command in ver4, and it is reserved or undocumented
static cerr_t amd_hsmp_test_ver5(bool *ok) { return amd_hsmp_test_ver(ok, 5); }
static cerr_t amd_hsmp_test_fail(bool *ok) { *ok = false; return NULL; } // HSMP commandss > 2Fh are not yet documented and do not seem to work by tody (Bergamo + Genoa)
...
static cerr_t get_real_apic_id(uint32_t *real_apic_id, const HWThread *hwt)
{
    ...
    if (err != 0)
        return ERROR_SET_ERRNO("sched_getaffinity failed");
    ...
    if (err != 0)
        return ERROR_SET_ERRNO("sched_setaffinity failed");
...
static cerr_t amd_hsmp_core_boost_limit_getter(LikwidDevice_t dev, char **value)
{
...
static cerr_t amd_hsmp_dimm_temp_getter(LikwidDevice_t dev, uint32_t channel, bool dimm_0_1, bool sensor_0_1, bool rate, bool test, char **value)
{
    ...
    if (field32(range_raw, 0, 3) != 0x1 && field32(range_raw, 0, 3) != 0x5)
        return ERROR_SET("AMD HSMP: received invalid or unknown temperature range: %x", field32(range_raw, 0, 3));
...
static cerr_t amd_hsmp_dimm_power_getter(LikwidDevice_t dev, uint32_t channel, bool dimm_0_1, bool sensor_0_1, bool test, char **value)
{
    /* See notes from hsmp_amd_dimm_temp_getter */
    assert(channel < 16);
    const uint32_t dimm_addr = make_dimm_addr(channel, dimm_0_1, sensor_0_1);
    uint32_t power;
    if (hsmp_raw(dev->id.simple.id, HSMP_GET_DIMM_THERMAL, &dimm_addr, 1, &power, 1))
...
static cerr_t amd_hsmp_dimm_tester(bool *ok, uint32_t channel, bool dimm_0_1, bool sensor_0_1, bool temp)
{
    ...
    for (uint32_t i = 0; i < topo->numSockets; i++)
    {
        ...
        if (err < 0)
            return ERROR_SET_LWERR(err, "likwid_device_create failed");

        cerr_t cerr;
        if (temp)
            cerr = amd_hsmp_dimm_temp_getter(dev, channel, dimm_0_1, sensor_0_1, false, true, NULL);
        else
            cerr = amd_hsmp_dimm_power_getter(dev, channel, dimm_0_1, sensor_0_1, true, NULL);
...
#define MAKE_DIMM_FUNC(channel, dimm, sensor)                               \
    static cerr_t amd_hsmp_dimm##channel##_##dimm##_ts##sensor##_temp_getter(LikwidDevice_t dev, char **value) \
    {                                                                       \
        return amd_hsmp_dimm_temp_getter(dev, channel, dimm, sensor, false, false, value); \
    }                                                                       \
    static cerr_t amd_hsmp_dimm##channel##_##dimm##_ts##sensor##_temp_rate_getter(LikwidDevice_t dev, char **value) \
    {                                                                       \
        return amd_hsmp_dimm_temp_getter(dev, channel, dimm, sensor, true, false, value);   \
    }                                                                       \
    static cerr_t amd_hsmp_dimm##channel##_##dimm##_ts##sensor##_temp_tester(bool *ok) \
    {                                                                       \
        return amd_hsmp_dimm_tester(ok, channel, dimm, sensor, true);           \
    }                                                                       \
    static cerr_t amd_hsmp_dimm##channel##_##dimm##_ts##sensor##_power_getter(LikwidDevice_t dev, char **value) \
    {                                                                       \
        return amd_hsmp_dimm_power_getter(dev, channel, dimm, sensor, false, value); \
    }                                                                       \
    static cerr_t amd_hsmp_dimm##channel##_##dimm##_ts##sensor##_power_tester(bool *ok) \
    {                                                                       \
        return amd_hsmp_dimm_tester(ok, channel, dimm, sensor, false);          \
    }
// clang-format on
...
static cerr_t amd_hsmp_sock_freq_limit_getter(LikwidDevice_t dev, bool show_reason, char **value)
{
    uint32_t freq;
    ...
    if (!reasons)
        return ERROR_SET("bfromcstr failed");
...
static cerr_t amd_hsmp_sock_xgmi_bw_getter(LikwidDevice_t dev, XGMILinkId id, XGMIBw bw, char **value)
{
    uint32_t arg0 = 0;
...
static cerr_t amd_hsmp_power_mode_getter(LikwidDevice_t dev, char **value)
{
...
static cerr_t amd_hsmp_power_mode_setter(LikwidDevice_t dev, const char *value)
{
    ...
    if (!found)
        return ERROR_SET("Cannot set power mode to '%s', which we don't know", value);
...
static _SysFeature amd_hsmp_features[] = {
    ...
    {"pkg_prochot", "hsmp", "Processor hot (throttling?)", amd_hsmp_sock_proc_hot_getter, NULL, DEVICE_TYPE_SOCKET, amd_hsmp_test_ver1, NULL},
    {"pkg_xgmi_link_width", "hsmp", "xGMI Link width range ([15:8] = min, [7:0] = max, 0 = x4, 1 = x8, 2 = x16", NULL, amd_hsmp_xgmi_link_width_setter, DEVICE_TYPE_NODE, amd_hsmp_test_ver1, NULL},
    {"pkg_df_pstate", "hsmp", "Disable AMD Precision Boost (override P-State: 0 = high performance .. 2 = low performance)", NULL, amd_hsmp_df_pstate_setter, DEVICE_TYPE_SOCKET, amd_hsmp_test_ver1, NULL},
    {"pkg_df_pstate_auto", "hsmp", "Enable AMD Precision Boost (auto manage P-State)", NULL, amd_hsmp_auto_df_pstate_setter, DEVICE_TYPE_SOCKET, amd_hsmp_test_ver1, NULL},
    ...
    {"pkg_c0", "hsmp", "Average C0 residency", amd_hsmp_sock_c0_percent_getter, NULL, DEVICE_TYPE_SOCKET, amd_hsmp_test_ver1, NULL},
    {"pkg_lclk_dpm_level_min_max", "hsmp", "Set LCLK DPM Level ([23:16] = NBIO ID (0..3), [15:8] = max DPM, [7:0] = min DPM, 0 = lowest DPM freq, 1..3 = highest DPM freq)", NULL, amd_hsmp_sock_lclk_dpm_setter, DEVICE_TYPE_SOCKET, amd_hsmp_test_ver2, NULL},
    ...
    {"pkg_freq_limit_reason", "hsmp", "Current socket frequency limit reason", amd_hsmp_sock_freq_limit_reason_getter, NULL, DEVICE_TYPE_SOCKET, amd_hsmp_test_ver5, NULL},
    ...
    {"pkg_gmi3_link_width", "hsmp", "Set minimum and maximum GMI3 link width (bitfield integer: min[15:8], max[7:0], 0 = quarter, 1 = half, 2 = full)", NULL, amd_hsmp_gmi3_width_setter, DEVICE_TYPE_SOCKET, amd_hsmp_test_ver5, NULL},
    {"pkg_pcie_gen_limit", "hsmp", "Set maximum PCIe gen (0 = auto, 1 = gen4, 2 = gen5)", NULL, amd_hsmp_pci_gen_limit_setter, DEVICE_TYPE_SOCKET, amd_hsmp_test_ver5, NULL},
    {"pkg_power_mode", "hsmp", "Current power mode", amd_hsmp_power_mode_getter, amd_hsmp_power_mode_setter, DEVICE_TYPE_SOCKET, amd_hsmp_test_ver5, NULL},
    {"pkg_pstate_min_max", "hsmp", "Set minimum and maximum Pstate ([15:8] = min, [7:0] = max, valid values: 0 = high performance .. 2 = low performance)", NULL, amd_hsmp_pstate_min_max_setter, DEVICE_TYPE_SOCKET, amd_hsmp_test_ver5, NULL},
    {"pkg_metric_table_ver", "hsmp", "Metric Table Version (?)", amd_hsmp_metric_table_ver_getter, NULL, DEVICE_TYPE_SOCKET, amd_hsmp_test_ver5, NULL},
    {"pkg_metric_table_addr", "hsmp", "Metric Table DRAM address (?)", amd_hsmp_metric_table_addr_getter, NULL, DEVICE_TYPE_SOCKET, amd_hsmp_test_ver5, NULL},
    {"pkg_xgmi_pstate_min_max", "hsmp", "Set minimum and maximum xGMI Pstate ([15:8] = min, [7:0] = max)", NULL, amd_hsmp_xgmi_pstate_min_max_setter, DEVICE_TYPE_SOCKET, amd_hsmp_test_ver5, NULL},
    {"pkg_rail_iso_freq_pol", "hsmp", "CPU Rail Iso Freq Policy (?)", amd_hsmp_cpu_rail_iso_freq_policy_getter, amd_hsmp_cpu_rail_iso_freq_policy_setter, DEVICE_TYPE_SOCKET, amd_hsmp_test_ver5, NULL},
    {"pkg_dfc_enable", "hsmp", "DFC Enable (?)", amd_hsmp_dfc_enable_getter, amd_hsmp_dfc_enable_setter, DEVICE_TYPE_SOCKET, amd_hsmp_test_ver5, NULL},
