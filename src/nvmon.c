...
int nvmon_init(int nrGpus, const int *gpuIds)
{
    ...
    for (int i = 0; i < nrGpus; i++) {
        ...
        if (gtopo->devices[gpuIds[i]].ccapMajor < 7) {
            GPUDEBUG_PRINT(
                DEBUGLEV_DEVELOP, "Device %d runs with CUPTI Event API backend", gpuIds[i]);
            device->backend = LIKWID_NVMON_CUPTI_BACKEND;
            cputicount++;
        } else {
            GPUDEBUG_PRINT(
                DEBUGLEV_DEVELOP, "Device %d runs with CUPTI Profiling API backend", gpuIds[i]);
...
static int nvmon_splitEventSet(GroupInfo *backendEvents, GroupInfo *nvmlEvents, int gid)
{
    int ret;
...
static void nvmon_returnSplitEventSet(GroupInfo *backendEvents, GroupInfo *nvmlEvents)
{
...
static int nvmon_initEventSourceLookupMaps(int gid, int gpuId)
{
    int ret;
    ...
    if (gid >= nvGroupSet->numGroupSources) {
        ...
        NvmonGroupSourceInfo *tmpInfo = (NvmonGroupSourceInfo *)realloc(
            nvGroupSet->groupSources, (gid + 1) * sizeof(NvmonGroupSourceInfo));
        if (tmpInfo == NULL) {
...
int nvmon_addEventSet(const char *eventCString)
{
    ...
    if (nvGroupSet->numberOfActiveGroups == nvGroupSet->numberOfGroups) {
        GroupInfo *tmpInfo = (GroupInfo *)realloc(
            nvGroupSet->groups, (nvGroupSet->numberOfGroups + 1) * sizeof(GroupInfo));
        if (tmpInfo == NULL) {
    ...
    GPUDEBUG_PRINT(DEBUGLEV_INFO,
        "NVMON: Currently %d groups of %d active",
    ...
    if (bstrchrp(eventBString, ':', 0) != BSTR_ERR) {
        ...
        err = perfgroup_customGroup(
            eventCString, &nvGroupSet->groups[nvGroupSet->numberOfGroups - 1]);
        if (err) {
    ...
    // Build source lookup
    err = nvmon_initEventSourceLookupMaps(nvGroupSet->numberOfGroups - 1,
        nvGroupSet->gpus[0].deviceId); // it is assumed that all gpus split the same
    if (err < 0) {
...
{
    if ((!nvGroupSet) || (!nvmon_initialized) || (groupId < 0) ||
        groupId >= nvGroupSet->numberOfActiveGroups) {
        return -EFAULT;
    }
...
double nvmon_getTimeOfGroup(int groupId)
{
    ...
    for (int i = 0; i < nvGroupSet->numberOfGPUs; i++) {
        ...
        backendTime = MAX(
            backendTime, (double)(nvGroupSet->gpus[i].timeStop - nvGroupSet->gpus[i].timeStart));
    }
...
double nvmon_getLastTimeOfGroup(int groupId)
{
    ...
    for (int i = 0; i < nvGroupSet->numberOfGPUs; i++) {
        // timeStart and timeStop are zero if no backend event is registered
        backendTime =
            MAX(backendTime, (double)(nvGroupSet->gpus[i].timeStop - nvGroupSet->gpus[i].timeRead));
    }
    backendTime *= 1E-9; // ns to seconds
...
double nvmon_getTimeToLastReadOfGroup(int groupId)
{
    ...
    for (int i = 0; i < nvGroupSet->numberOfGPUs; i++) {
        // timeStart and timeStop are zero if no backend event is registered
        backendTime = MAX(
            backendTime, (double)(nvGroupSet->gpus[i].timeRead - nvGroupSet->gpus[i].timeStart));
    }
    backendTime *= 1E-9; // ns to seconds
...
int nvmon_getGroups(int gpuId, char ***groups, char ***shortinfos, char ***longinfos)
{
    ...
    if (gtopo->devices[gpuId].ccapMajor >= 7) {
        ret = perfgroup_getGroups(
            config->groupPath, "nvidia_gpu_cc_ge_7", groups, shortinfos, longinfos);
    } else {
        ret = perfgroup_getGroups(
            config->groupPath, "nvidia_gpu_cc_lt_7", groups, shortinfos, longinfos);
    }
    return ret;
}
...
void nvmon_destroyMarkerResults()
{
    int i = 0, j = 0;
    if (gMarkerResults != NULL) {
        for (i = 0; i < gMarkerRegions; i++) {
...
int nvmon_getGpusOfRegion(int region)
{
    if (gMarkerResults == NULL) {
        ERROR_PRINT("Nvmon module not properly initialized");
        return -EINVAL;
...
int nvmon_getMetricsOfRegion(int region)
{
    if (gMarkerResults == NULL) {
        ERROR_PRINT("Nvmon module not properly initialized");
        return -EINVAL;
...
int nvmon_getGroupOfRegion(int region)
{
    if (gMarkerResults == NULL) {
        ERROR_PRINT("Nvmon module not properly initialized");
        return -EINVAL;
...
char *nvmon_getTagOfRegion(int region)
{
    if (gMarkerResults == NULL) {
        ERROR_PRINT("Nvmon module not properly initialized");
        return NULL;
...
int nvmon_getEventsOfRegion(int region)
{
    if (gMarkerResults == NULL) {
        ERROR_PRINT("Nvmon module not properly initialized");
        return -EINVAL;
