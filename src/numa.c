...
uint64_t proc_getFreeSysMem(void)
{
    FILE *fp;
    uint64_t free      = 0;
...
uint64_t proc_getTotalSysMem(void)
{
    int i;
    FILE *fp;
    uint64_t total      = 0;
...
void empty_numa_setInterleaved(const int *processorList, int numberOfProcessors)
{
    printf("MEMPOLICY NOT supported in kernel!\n");
...
void empty_numa_setMembind(const int *processorList, int numberOfProcessors)
{
    printf("MEMPOLICY NOT supported in kernel!\n");
    if (!processorList || numberOfProcessors < 0)
        printf("Invalid options\n");
...
void empty_numa_membind(void *ptr, size_t size, int domainId)
{
...
void numa_setMembind(const int *processorList, int numberOfProcessors)
{
    const struct numa_functions funcs = numa_funcs;
...
#ifndef HAS_MEMPOLICY
void numa_finalize(void) { return; }
#else
