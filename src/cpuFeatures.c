...
void cpuFeatures_print(int cpu)
{
    ...
    for (int i = 0; i < CPUFEATURES_MAX; i++) {
        if ((cpuid_info.model != CORE2_45) && (cpuid_info.model != CORE2_65) && ((i == FEAT_FERR_MULTIPLEX) || (i == FEAT_DYN_ACCEL) || (i == FEAT_SPEEDSTEP_LOCK) || (i == FEAT_TM2))) {
            continue;
        }
