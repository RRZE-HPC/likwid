    ...
    for (int i = 0; i < groupSet->groups[groupSet->activeGroup].numberOfEvents; i++) {
        if (groupSet->groups[groupSet->activeGroup].events[i].type != NOTYPE) {
            result = calculateMarkerResult(groupSet->groups[groupSet->activeGroup].events[i].index, results->StartPMcounters[i],
                groupSet->groups[groupSet->activeGroup].events[i].threadCounter[thread_id].counterData,
                groupSet->groups[groupSet->activeGroup].events[i].threadCounter[thread_id].overflows - results->StartOverflows[i]);
            ...
            if ((counter_map[groupSet->groups[groupSet->activeGroup].events[i].index].type != THERMAL) && (counter_map[groupSet->groups[groupSet->activeGroup].events[i].index].type != VOLTAGE) && (counter_map[groupSet->groups[groupSet->activeGroup].events[i].index].type != MBOX0TMP)) {
