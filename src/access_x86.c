...
int access_x86_read(PciDeviceIndex dev, const int cpu_id, uint32_t reg, uint64_t *data)
{
    ...
    if (dev == MSR_DEV) {
        ...
        *data = tmp;
        ...
        if (cpuid_info.supportUncore) {
            if (((dev >= MMIO_IMC_DEVICE_0_CH_0 && dev <= MMIO_IMC_DEVICE_3_CH_1) || (dev >= MMIO_IMC_DEVICE_0_FREERUN && dev <= MMIO_IMC_DEVICE_3_FREERUN)) && (cpuid_info.family == P6_FAMILY && ((cpuid_info.model == ICELAKEX1) || (cpuid_info.model == ICELAKEX2)))) {
        ...
        } else if (cpuid_info.supportClientmem && dev == PCI_IMC_DEVICE_0_CH_0 && access_x86_clientmem_check(dev, affinity_thread2socket_lookup[cpu_id])) {
