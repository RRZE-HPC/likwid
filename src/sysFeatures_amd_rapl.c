...
{
    (void)value;
    if (amd_rapl_l3_info.powerUnit == 0 && amd_rapl_l3_info.energyUnit == 0 &&
        amd_rapl_l3_info.timeUnit == 0) {
...
static int amd_rapl_l3_test(void)
{
    ...
    if (info->model != ZEN4_RYZEN && info->model != ZEN4_RYZEN_PRO && info->model != ZEN4_EPYC &&
        info->model != ZEN4_RYZEN2 && info->model != ZEN4_RYZEN3)
        return 0;
...
{
    return amd_rapl_energy_status_getter(
        device, value, MSR_AMD19_RAPL_L3_STATUS, &amd_rapl_l3_info, DEVICE_TYPE_SOCKET);
}

static _SysFeature amd_rapl_l3_features[] = {
    { "l3_energy",
     "rapl", "Current energy consumtion (L3 domain)",
     amd_l3_energy_status_getter, NULL,
     DEVICE_TYPE_SOCKET, amd_l3_energy_status_test,
     "J" },
};
