...
void segfault_sigaction_rdpmc(int signal, siginfo_t *si, void *arg)
{
    (void)signal;
    (void)si;
    (void)arg;
    ...
    if (cpuid_info.isIntel) {
        ...
        if (rdpmc_works_llc < 0) {
            ...
            case ZEN_FAMILY:
                ...
                DEBUG_PRINT(DEBUGLEV_DEVELOP,
                    "Test for RDPMC for L3 counters returned %d",
                    rdpmc_works_llc);
                break;
            case ZEN3_FAMILY:
                rdpmc_works_llc = test_rdpmc(cpu_id, 0xA, 0);
...
int access_x86_rdpmc_read(uint32_t cpu_id, uint32_t reg, uint64_t *data)
{
    int ret = -EAGAIN;
...
int access_x86_rdpmc_write(uint32_t cpu_id, uint32_t reg, uint64_t data)
{
    (void)cpu_id;
    (void)reg;
    (void)data;
