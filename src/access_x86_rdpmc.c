...
void segfault_sigaction_rdpmc(int signal, siginfo_t *si, void *arg) { quick_exit(1); }

static int test_rdpmc(int cpu_id, uint64_t value, int flag)
{
    int ret = -1;
    int pid;
...
int access_x86_rdpmc_init(const int cpu_id)
{
    ...
    if (rdpmc_works_pmc < 0) {
        ...
        DEBUG_PRINT(
            DEBUGLEV_DEVELOP, "Test for RDPMC for PMC counters returned %d", rdpmc_works_pmc);
    }
    if (rdpmc_works_fixed_inst < 0 && cpuid_info.isIntel) {
        if (eventSupportedCount > 1 && (!(ebx & (1 << 1)))) {
            ...
            DEBUG_PRINT(DEBUGLEV_DEVELOP,
                "Test for RDPMC for FIXED instruction counter returned %d",
                rdpmc_works_fixed_inst);
        }
    }
    if (rdpmc_works_fixed_cyc < 0 && cpuid_info.isIntel) {
        if (eventSupportedCount > 0 && (!(ebx & (1 << 0)))) {
            ...
            DEBUG_PRINT(DEBUGLEV_DEVELOP,
                "Test for RDPMC for FIXED core cycles counter returned %d",
                rdpmc_works_fixed_cyc);
        }
    }
    if (rdpmc_works_fixed_ref < 0 && cpuid_info.isIntel) {
        if (eventSupportedCount > 2 && (!(ebx & (1 << 2)))) {
            ...
            DEBUG_PRINT(DEBUGLEV_DEVELOP,
                "Test for RDPMC for FIXED reference cycle counter returned %d",
                rdpmc_works_fixed_ref);
        }
    }
    if (rdpmc_works_fixed_slots < 0 && cpuid_info.isIntel) {
        if (eventSupportedCount > 7 && (!(ebx & (1 << 7)))) {
            ...
            DEBUG_PRINT(DEBUGLEV_DEVELOP,
                "Test for RDPMC for FIXED slots counter returned %d",
    ...
    if (rdpmc_works_llc < 0 && (!cpuid_info.isIntel)) {
        ...
        case 0x17:
            ...
            DEBUG_PRINT(
                DEBUGLEV_DEVELOP, "Test for RDPMC for L3 counters returned %d", rdpmc_works_llc);
            break;
        case 0x19:
            rdpmc_works_llc = test_rdpmc(cpu_id, 0xA, 0);
            DEBUG_PRINT(
    ...
    if (rdpmc_works_mem < 0) {
        ...
        case 0x17:
            ...
            DEBUG_PRINT(DEBUGLEV_DEVELOP,
                "Test for RDPMC for DataFabric counters returned %d",
                rdpmc_works_mem);
            break;
        case 0x19:
            rdpmc_works_mem = test_rdpmc(cpu_id, 0x6, 0);
...
int access_x86_rdpmc_read(const int cpu_id, uint32_t reg, uint64_t *data)
{
    int ret = -EAGAIN;
    ...
    case MSR_PMC7:
        if (rdpmc_works_pmc == 1) {
            DEBUG_PRINT(DEBUGLEV_DEVELOP,
                "Read PMC counter with RDPMC instruction with index 0x%X",
                reg - MSR_PMC0);
    ...
    case MSR_AMD17_PMC3:
        if (rdpmc_works_pmc == 1 && !cpuid_info.isIntel) {
            ...
            DEBUG_PRINT(
                DEBUGLEV_DEVELOP, "Read PMC counter with RDPMC instruction with index 0x%X", index);
    ...
    case MSR_PERF_FIXED_CTR0:
        if (rdpmc_works_fixed_inst == 1) {
            DEBUG_PRINT(DEBUGLEV_DEVELOP,
                "Read FIXED instruction counter with RDPMC instruction with index 0x%X",
                (1 << 30) + (reg - MSR_PERF_FIXED_CTR0));
    ...
    case MSR_PERF_FIXED_CTR1:
        if (rdpmc_works_fixed_cyc == 1) {
            DEBUG_PRINT(DEBUGLEV_DEVELOP,
                "Read FIXED core cycle counter with RDPMC instruction with index 0x%X",
                (1 << 30) + (reg - MSR_PERF_FIXED_CTR0));
            ret = __rdpmc(cpu_id, (1 << 30) + (reg - MSR_PERF_FIXED_CTR0), data);
            if (ret) {
    ...
    case MSR_PERF_FIXED_CTR2:
        if (rdpmc_works_fixed_ref == 1) {
            DEBUG_PRINT(DEBUGLEV_DEVELOP,
                "Read FIXED reference cycle counter with RDPMC instruction with index 0x%X",
                (1 << 30) + (reg - MSR_PERF_FIXED_CTR0));
            ret = __rdpmc(cpu_id, (1 << 30) + (reg - MSR_PERF_FIXED_CTR0), data);
            if (ret) {
    ...
    case MSR_PERF_FIXED_CTR3: //Fixed-purpose counter for TOPDOWN_SLOTS is not readable with RDPMC
        if (rdpmc_works_fixed_slots == 1) {
            DEBUG_PRINT(DEBUGLEV_DEVELOP,
                "Read FIXED slots counter with RDPMC instruction with index 0x%X",
                (1 << 30) + (reg - MSR_PERF_FIXED_CTR0));
            ret = __rdpmc(cpu_id, (1 << 30) + (reg - MSR_PERF_FIXED_CTR0), data);
            if (ret) {
    ...
    case MSR_AMD17_L3_PMC5:
        if (rdpmc_works_llc == 1) {
            ...
            DEBUG_PRINT(DEBUGLEV_DEVELOP,
                "Read AMD L3 counter with RDPMC instruction with index 0x%X",
                0xA + index);
    ...
    case MSR_AMD17_2_DF_PMC3:
        if (rdpmc_works_mem == 1) {
            ...
            DEBUG_PRINT(DEBUGLEV_DEVELOP,
                "Read AMD DF counter with RDPMC instruction with index 0x%X",
                0x6 + index);
...
int access_x86_rdpmc_write(const int cpu_id, uint32_t reg, uint64_t data) { return -EPERM; }
