...
int proc_init_cpuFeatures(void)
{
    ...
    for (int i = 0; i < cpulines->qty; i++) {
        ...
        if (bstrncmp(cpulines->entry[i], &flagString, 5) == BSTR_OK || bstrncmp(cpulines->entry[i], &featString, 8) == BSTR_OK)
#endif
...
void proc_split_llc_check(CacheLevel *llc_cache)
{
    ...
    while ((ep = readdir(dp))) {
        if (strncmp(ep->d_name, "cpu", 3) == 0 && ep->d_name[strlen(ep->d_name) - 1] >= '0' && ep->d_name[strlen(ep->d_name) - 1] <= '9') {
    ...
    for (int i = 0; i < nrCaches; i++) {
        ...
        if (NULL != (fp = fopen(bdata(levelStr), "r"))) {
            ...
            if ((cachePool[i].associativity * cachePool[i].lineSize) != 0) {
                cachePool[i].sets = cachePool[i].size / (cachePool[i].associativity * cachePool[i].lineSize);
            }
        }
        bdestroy(levelStr);
