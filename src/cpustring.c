...
static int cpustr_to_cpulist_expression(bstring bcpustr, int *cpulist, int length)
{
    ...
    for (int i = 0; i < count; i++) {
        for (unsigned j = 0;
            j < (unsigned)chunk && offset + j < affinity->domains[domainidx].numberOfProcessors;
            j++) {
    ...
    for (int i = 0; i < strlist->qty; i++) {
        if (bstrchrp(strlist->entry[i], '-', 0) != BSTR_ERR) {
            ...
            if (cpu < 0) {
                ...
                int notInCpuSet = 0;
                for (unsigned k = 0; k < cpuid_topology->numHWThreads; k++) {
                    if (cpuid_topology->threadPool[k].apicId == (unsigned)cpu &&
                        !cpuid_topology->threadPool[k].inCpuSet) {
                        notInCpuSet = 1;
                    }
                }
...
int gpustr_to_gpulist_rocm(const char *gpustr, int **gpuIds, size_t *numGpus)
{
    bstring bgpustr            = bfromcstr(gpustr);
    struct bstrList *commalist = bsplit(bgpustr, ',');
    bdestroy(bgpustr);
