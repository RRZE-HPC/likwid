...
static int check_and_atoi(char *s)
{
    int i   = 0;
    int len = strlen(s);
    for (i = 0; i < len; i++) {
...
static int get_domain_idx(char *bdomain)
{
    affinity_init();
    AffinityDomains_t affinity = get_affinityDomains();
    for (int i = 0; i < affinity->numberOfAffinityDomains; i++) {
        if (strncmp(affinity->domains[i].tag, bdomain, strlen(affinity->domains[i].tag)) ==
            BSTR_OK) {
            return i;
        }
    }
...
static int cpuexpr_to_list(bstring bcpustr, bstring prefix, int *list, int length)
{
    ...
    for (int i = 0; i < strlist->qty; i++) {
        ...
        for (int j = 0; j < affinity->numberOfAffinityDomains; j++) {
            if (strncmp(affinity->domains[j].tag,
                    bdata(newstr),
                    strlen(affinity->domains[j].tag)) == 0) {
...
static int cpustr_to_cpulist_method(bstring bcpustr, int *cpulist, int length)
{
    int max_procs   = 0;
    int given_procs = 0;
    topology_init();
    ...
    if (bstrchrp(bcpustr, ':', 0) != BSTR_ERR) {
        ...
        for (int i = 0; i < suitidx; i++) {
            ...
            cpulist_sort(affinity->domains[suitable[i]].processorList,
                sLists[i],
                affinity->domains[suitable[i]].numberOfProcessors);
        }
...
static int cpustr_to_cpulist_expression(bstring bcpustr, int *cpulist, int length)
{
    ...
    for (int i = 0; i < count; i++) {
        for (int j = 0; j < chunk && offset + j < affinity->domains[domainidx].numberOfProcessors;
            j++) {
...
static int cpustr_to_cpulist_logical(bstring bcpustr, int *cpulist, int length)
{
    ...
    if (strlist->qty != 3) {
        fprintf(stderr,
            "ERROR: Invalid expression, should look like <domain>:<indexlist> or "
            "L:<domain>:<indexlist>\n");
        bstrListDestroy(strlist);
        return 0;
    }
    ...
    int ret           = cpulist_sort(affinity->domains[domainidx].processorList,
        inlist,
        affinity->domains[domainidx].numberOfProcessors);
    ...
    for (int i = 0; i < strlist->qty; i++) {
        if (bstrchrp(strlist->entry[i], '-', 0) != BSTR_ERR) {
            ...
            if (start < 0 || end < 0) {
                fprintf(stderr,
                    "CPU expression %s contains non-numerical characters\n",
    ...
    if (require > ret && getenv("LIKWID_SILENT") == NULL) {
        fprintf(stderr,
            "WARN: Selected affinity domain %s has only %d hardware threads, but selection string "
            "evaluates to %d "
            "threads.\n",
            affinity->domains[domainidx].tag,
            ret,
            require);
    ...
    for (int i = 0; i < strlist->qty; i++) {
        if (bstrchrp(strlist->entry[i], '-', 0) != BSTR_ERR) {
                ...
                for (int j = start; j <= end; j++) {
                    if (cpu_in_domain(domainidx, j)) {
                        ...
                        for (int k = 0; k < cpuid_topology->numHWThreads; k++) {
                            if (cpuid_topology->threadPool[k].apicId == j &&
                                !cpuid_topology->threadPool[k].inCpuSet) {
                                notInCpuSet = 1;
                            }
                        }
            ...
            if (cpu < 0) {
                ...
                int notInCpuSet = 0;
                for (int k = 0; k < cpuid_topology->numHWThreads; k++) {
                    if (cpuid_topology->threadPool[k].apicId == cpu &&
                        !cpuid_topology->threadPool[k].inCpuSet) {
                        notInCpuSet = 1;
                    }
                }
    ...
    for (int i = 0; i < strlist->qty; i++) {
        if (binstr(strlist->entry[i], 0, scattercheck) != BSTR_ERR ||
            ...
            insert += cpulist_concat(cpulist, insert, tmpList, ret);
                 ...
                 (bstrchrp(strlist->entry[i], 'G', 0) == 0)) {
            if (bstrchrp(strlist->entry[i], ':', 0) != BSTR_ERR) {
            ...
            if (cpuid_topology->activeHWThreads < cpuid_topology->numHWThreads) {
                if (getenv("LIKWID_SILENT") == NULL) {
                    fprintf(stdout,
                        "INFO: You are running LIKWID in a cpuset with %d CPUs. Taking given IDs "
                        "as logical ID in "
                        "cpuset\n",
...
static int valid_gpu_cuda(CudaTopology_t topo, int id)
{
...
int gpustr_to_gpulist_cuda(const char *gpustr, int *gpulist, int length)
{
    int insert = 0;
    topology_cuda_init();
    CudaTopology_t gpu_topology = get_cudaTopology();
        ...
        if (bstrchrp(commalist->entry[i], '-', 0) != BSTR_ERR) {
            ...
            if (start <= end) {
                for (int k = start; k <= end; k++) {
                    if (valid_gpu_cuda(gpu_topology, k) && insert < length) {
                        gpulist[insert] = k;
                        insert++;
                    }
                }
            ...
            if (valid_gpu_cuda(gpu_topology, id) && insert < length) {
                gpulist[insert] = id;
                insert++;
            }
        }
...
int gpustr_to_gpulist_rocm(const char *gpustr, int *gpulist, int length)
{
    int insert = 0;
    topology_rocm_init();
