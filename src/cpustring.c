...
cpustr_to_cpulist_method(bstring bcpustr, int *cpulist, int length)
{
    ...
    if (bstrchrp(bcpustr, ':', 0) != BSTR_ERR) {
        ...
        for (int i = 0; i < affinity->numberOfAffinityDomains; i++) {
            if (strstr(affinity->domains[i].tag, bdata(parts->entry[0])) != NULL && affinity->domains[i].numberOfProcessors > 0) {
                suitable[suitidx] = i;
                suitidx++;
...
int cpustr_to_cpulist(const char *cpustring, int *cpulist, int length)
{
    ...
    for (int i = 0; i < strlist->qty; i++) {
        if (binstr(strlist->entry[i], 0, scattercheck) != BSTR_ERR || binstr(strlist->entry[i], 0, balancedcheck) != BSTR_ERR) {
        ...
        else if ((bstrchrp(strlist->entry[i], 'N', 0) == 0) || (bstrchrp(strlist->entry[i], 'S', 0) == 0) || (bstrchrp(strlist->entry[i], 'C', 0) == 0) || (bstrchrp(strlist->entry[i], 'M', 0) == 0) || (bstrchrp(strlist->entry[i], 'D', 0) == 0) || (bstrchrp(strlist->entry[i], 'G', 0) == 0)) {
            if (bstrchrp(strlist->entry[i], ':', 0) != BSTR_ERR) {
