...
_timer_printCycles(const TimerData *time)
{
    ...
    if (((time->stop.int64 - baseline) < time->start.int64) || (time->start.int64 == time->stop.int64)) {
        cycles = 0ULL;
    } else {
...
getCpuSpeed(void)
{
    ...
    if (eax >= 0x16) {
        ...
        cpuClock = (result) * 1000000 / (((uint64_t)tv2.tv_sec * 1000000 + tv2.tv_usec) - ((uint64_t)tv1.tv_sec * 1000000 + tv1.tv_usec));
    }
