    ...
    for (uint32_t i = 0; i < cpuid_topology.numHWThreads; i++) {
        ...
        if (maxNumLogicalProcsPerCore == 1 && cpuid_info.isIntel == 0) {
            if (id == 0) {
                ...
                if (hwThreadPool[id].apicId == hwThreadPool[id - 1].apicId + 1 && hwThreadPool[id].packageId == hwThreadPool[id - 1].packageId) {
...
int hwloc_init_cacheTopology(void)
{
    ...
    for (d = 0; d < depth; d++) {
        ...
        if (depth_type == HWLOC_OBJ_L1CACHE || depth_type == HWLOC_OBJ_L2CACHE || depth_type == HWLOC_OBJ_L3CACHE || depth_type == HWLOC_OBJ_L4CACHE || depth_type == HWLOC_OBJ_L5CACHE)
            maxNumLevels++;
#else
    ...
    for (d = depth - 1; d >= 0; d--) {
        ...
        if ((cachePool[id].associativity * cachePool[id].lineSize) != 0) {
            cachePool[id].sets = cachePool[id].size / (cachePool[id].associativity * cachePool[id].lineSize);
        }
