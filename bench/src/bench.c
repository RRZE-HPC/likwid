...
void *runTest(void *arg)
{
    ...
    if (size != vecsize && data->threadId == 0)
        printf("Sanitizing vector length to a multiple of the loop stride from %zu elements (%zu "
               "bytes) to %zu elements (%zu bytes)\n",
            vecsize,
            vecsize * myData->test->bytes,
            size,
            size * myData->test->bytes);

    /* pin the thread */
    likwid_pinThread(myData->processors[threadId]);
    printf("Group: %d Thread %d Global Thread %d running on hwthread %d - Vector length %llu "
           "Offset %zd\n",
        data->groupId,
        threadId,
        data->globalThreadId,
    ...
    case STREAM_4:
        EXECUTE(func(
            size, myData->streams[0], myData->streams[1], myData->streams[2], myData->streams[3]));
        break;
    case STREAM_5:
        EXECUTE(func(size,
            myData->streams[0],
